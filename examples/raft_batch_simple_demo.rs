...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Total batches: {}, Total proposals: {}",
        batch_count, total_proposals
    );
    info!(
        "Average proposals per batch: {:.1}",
        total_proposals as f64 / batch_count as f64
    );
