...
-opaque decoder(DOW) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {DOW, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(DPE)) -> {ok, DPE} |
...
-spec map(decoder(DTC), fun((DTC) -> DTE)) -> decoder(DTE).
...
-spec then(decoder(DTO), fun((DTO) -> decoder(DTQ))) -> decoder(DTQ).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(DTT), list(decoder(DTT))) -> decoder(DTT).
...
-spec recursive(fun(() -> decoder(DUJ))) -> decoder(DUJ).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(DSY)) -> decoder(gleam@option:option(DSY)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, DQZ} | {error, DQZ})
) -> {DQZ, list(decode_error())}.
...
-spec collapse_errors(decoder(DTL), binary()) -> decoder(DTL).
...
-spec failure(DUD, binary()) -> decoder(DUD).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, DUF} | {error, DUF})
) -> decoder(DUF).
...
-spec fold_dict(
    {gleam@dict:dict(DSK, DSL), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {DSK, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {DSL, list(decode_error())})
) -> {gleam@dict:dict(DSK, DSL), list(decode_error())}.
...
-spec dict(decoder(DSD), decoder(DSF)) -> decoder(gleam@dict:dict(DSD, DSF)).
dict(Key, Value) ->
...
-spec list(decoder(DRR)) -> decoder(list(DRR)).
list(Inner) ->
    {decoder,
        fun(Data) ->
...
-spec push_path({DQA, list(decode_error())}, list(any())) -> {DQA,
    list(decode_error())}.
...
-spec index(
    list(DPO),
    list(DPO),
    fun((gleam@dynamic:dynamic_()) -> {DPR, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(DPO)) -> {DPR, list(decode_error())})
) -> {DPR, list(decode_error())}.
...
-spec subfield(list(any()), decoder(DOZ), fun((DOZ) -> decoder(DPB))) -> decoder(DPB).
...
-spec at(list(any()), decoder(DPL)) -> decoder(DPL).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(DQJ), fun((DQJ) -> decoder(DQL))) -> decoder(DQL).
...
-spec optional_field(any(), DQP, decoder(DQP), fun((DQP) -> decoder(DQR))) -> decoder(DQR).
...
-spec optionally_at(list(any()), DQW, decoder(DQW)) -> decoder(DQW).
