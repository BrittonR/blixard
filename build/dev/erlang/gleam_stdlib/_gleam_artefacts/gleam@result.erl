...
-spec map({ok, BYY} | {error, BYZ}, fun((BYY) -> BZC)) -> {ok, BZC} |
    {error, BYZ}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, BZF} | {error, BZG}, fun((BZG) -> BZJ)) -> {ok, BZF} |
    {error, BZJ}.
...
-spec flatten({ok, {ok, BZM} | {error, BZN}} | {error, BZN}) -> {ok, BZM} |
    {error, BZN}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, BZU} | {error, BZV}, fun((BZU) -> {ok, BZY} | {error, BZV})) -> {ok,
        BZY} |
    {error, BZV}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CAD} | {error, CAE}, fun((CAD) -> {ok, CAH} | {error, CAE})) -> {ok,
        CAH} |
    {error, CAE}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CAM} | {error, any()}, CAM) -> CAM.
...
-spec lazy_unwrap({ok, CAQ} | {error, any()}, fun(() -> CAQ)) -> CAQ.
...
-spec unwrap_error({ok, any()} | {error, CAV}, CAV) -> CAV.
...
-spec unwrap_both({ok, CAY} | {error, CAY}) -> CAY.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CBB} | {error, CBC}, {ok, CBB} | {error, CBC}) -> {ok, CBB} |
    {error, CBC}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CBJ} | {error, CBK}, fun(() -> {ok, CBJ} | {error, CBK})) -> {ok,
        CBJ} |
    {error, CBK}.
...
-spec all(list({ok, CBR} | {error, CBS})) -> {ok, list(CBR)} | {error, CBS}.
all(Results) ->
    gleam@list:try_map(Results, fun(X) -> X end).

-file("src/gleam/result.gleam", 368).
-spec partition_loop(list({ok, CCG} | {error, CCH}), list(CCG), list(CCH)) -> {list(CCG),
    list(CCH)}.
...
-spec partition(list({ok, CBZ} | {error, CCA})) -> {list(CBZ), list(CCA)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CCP}, CCS) -> {ok, CCS} | {error, CCP}.
...
-spec replace_error({ok, CCV} | {error, any()}, CCZ) -> {ok, CCV} | {error, CCZ}.
...
-spec values(list({ok, CDC} | {error, any()})) -> list(CDC).
values(Results) ->
