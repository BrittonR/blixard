...
-opaque subject(FAE) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FAE}.

-type do_not_leak() :: any().

-type selector(FAF) :: any() | {gleam_phantom, FAF}.
...
-type call_error(FAG) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FAG}.
...
-file("src/gleam/erlang/process.gleam", 15).
?DOC(" Get the `Pid` for the current process.\n").
-spec self() -> pid_().
self() ->
    erlang:self().

-file("src/gleam/erlang/process.gleam", 29).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 111).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 190).
?DOC(
...
-spec select(selector(FAZ), integer()) -> {ok, FAZ} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-file("src/gleam/erlang/process.gleam", 199).
?DOC(
...
-spec select_forever(selector(FBD)) -> FBD.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-file("src/gleam/erlang/process.gleam", 208).
?DOC(
...
-spec map_selector(selector(FBF), fun((FBF) -> FBH)) -> selector(FBH).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-file("src/gleam/erlang/process.gleam", 217).
?DOC(
...
-spec merge_selector(selector(FBJ), selector(FBJ)) -> selector(FBJ).
merge_selector(A, B) ->
...
-file("src/gleam/erlang/process.gleam", 233).
?DOC(
...
-spec selecting_trapped_exits(selector(FBN), fun((exit_message()) -> FBN)) -> selector(FBN).
...
-file("src/gleam/erlang/process.gleam", 274).
?DOC(
...
-spec selecting(selector(FBQ), subject(FBS), fun((FBS) -> FBQ)) -> selector(FBQ).
...
-file("src/gleam/erlang/process.gleam", 300).
?DOC(
...
-spec selecting_record3(
    selector(FCE),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCE)
) -> selector(FCE).
...
-file("src/gleam/erlang/process.gleam", 334).
?DOC(
...
-spec selecting_record4(
    selector(FCI),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCI)
) -> selector(FCI).
...
-file("src/gleam/erlang/process.gleam", 352).
?DOC(
...
-spec selecting_record5(
    selector(FCM),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCM)
) -> selector(FCM).
...
-file("src/gleam/erlang/process.gleam", 370).
?DOC(
...
-spec selecting_record6(
    selector(FCQ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCQ)
) -> selector(FCQ).
...
-file("src/gleam/erlang/process.gleam", 388).
?DOC(
...
-spec selecting_record7(
    selector(FCU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCU)
) -> selector(FCU).
...
-file("src/gleam/erlang/process.gleam", 409).
?DOC(
...
-spec selecting_record8(
    selector(FCY),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FCY)
) -> selector(FCY).
...
-file("src/gleam/erlang/process.gleam", 459).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 483).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 490).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 499).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 530).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 541).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 555).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 580).
?DOC(
...
-spec deselecting_process_down(selector(FDR), process_monitor()) -> selector(FDR).
...
-spec try_call(subject(FDU), fun((subject(FDW)) -> FDU), integer()) -> {ok, FDW} |
    {error, call_error(FDW)}.
...
-file("src/gleam/erlang/process.gleam", 636).
?DOC(
    " Send a message to a process and wait for a reply.\n"
    "\n"
    " If the receiving process exits or does not reply within the allowed amount\n"
...
-spec call(subject(FEB), fun((subject(FED)) -> FEB), integer()) -> FED.
...
-file("src/gleam/erlang/process.gleam", 665).
?DOC(
...
-spec try_call_forever(subject(FEJ), fun((subject(FEL)) -> FEJ)) -> {ok, FEL} |
...
-file("src/gleam/erlang/process.gleam", 652).
?DOC(
...
-spec call_forever(subject(FEF), fun((subject(FEH)) -> FEF)) -> FEH.
...
-file("src/gleam/erlang/process.gleam", 703).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 710).
...
-spec send_after(subject(FES), integer(), FES) -> timer().
...
-file("src/gleam/erlang/process.gleam", 746).
...
-file("src/gleam/erlang/process.gleam", 770).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 786).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 798).
?DOC(
    " Sends an exit signal to a process, indicating that the process is to shut\n"
...
-file("src/gleam/erlang/process.gleam", 814).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 826).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 837).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 842).
