...
-type message(FPC) :: {message, FPC} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(FPD, FPE) :: {continue,
        FPE,
        gleam@option:option(gleam@erlang@process:selector(FPD))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(FPF, FPG) :: {ready, FPF, gleam@erlang@process:selector(FPG)} |
    {failed, binary()}.

-type self(FPH, FPI) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FPH,
        gleam@erlang@process:subject(FPI),
        gleam@erlang@process:selector(message(FPI)),
        gleam@otp@system:debug_state(),
        fun((FPI, FPH) -> next(FPI, FPH))}.

-type spec(FPJ, FPK) :: {spec,
        fun(() -> init_result(FPJ, FPK)),
        integer(),
        fun((FPK, FPJ) -> next(FPK, FPJ))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(FPL) :: {ack,
        {ok, gleam@erlang@process:subject(FPL)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(FPS) -> next(any(), FPS).
continue(State) ->
    {continue, State, none}.

-file("src/gleam/otp/actor.gleam", 186).
?DOC(
...
-spec with_selector(next(FPW, FPX), gleam@erlang@process:selector(FPW)) -> next(FPW, FPX).
...
-file("src/gleam/otp/actor.gleam", 259).
-spec exit_process(gleam@erlang@process:exit_reason()) -> gleam@erlang@process:exit_reason().
exit_process(Reason) ->
    Reason.

-file("src/gleam/otp/actor.gleam", 295).
-spec selecting_system_messages(gleam@erlang@process:selector(message(FQI))) -> gleam@erlang@process:selector(message(FQI)).
...
-file("src/gleam/otp/actor.gleam", 264).
-spec receive_message(self(any(), FQE)) -> message(FQE).
...
-file("src/gleam/otp/actor.gleam", 308).
...
-file("src/gleam/otp/actor.gleam", 415).
-spec init_selector(
    gleam@erlang@process:subject(FVI),
    gleam@erlang@process:selector(FVI)
) -> gleam@erlang@process:selector(message(FVI)).
...
-file("src/gleam/otp/actor.gleam", 318).
...
-file("src/gleam/otp/actor.gleam", 374).
-spec initialise_actor(
    spec(any(), FQZ),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(FQZ)} |
        {error, gleam@erlang@process:exit_reason()})
...
-file("src/gleam/otp/actor.gleam", 444).
?DOC(
...
-file("src/gleam/otp/actor.gleam", 466).
?DOC(
...
-spec start_spec(spec(any(), FRM)) -> {ok, gleam@erlang@process:subject(FRM)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-file("src/gleam/otp/actor.gleam", 517).
?DOC(
...
-spec start(FRS, fun((FRT, FRS) -> next(FRT, FRS))) -> {ok,
        gleam@erlang@process:subject(FRT)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec call(
    gleam@erlang@process:subject(FSB),
    fun((gleam@erlang@process:subject(FSD)) -> FSB),
    integer()
) -> FSD.
