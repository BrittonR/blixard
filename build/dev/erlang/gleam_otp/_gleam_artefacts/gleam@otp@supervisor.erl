...
-type spec(GFJ, GFK) :: {spec,
        GFJ,
        integer(),
        integer(),
        fun((children(GFJ)) -> children(GFK))}.

-opaque children(GFL) :: {ready, starter(GFL)} | {failed, child_start_error()}.

-opaque child_spec(GFM, GFN, GFO) :: {child_spec,
        fun((GFN) -> {ok, gleam@erlang@process:subject(GFM)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GFN, gleam@erlang@process:subject(GFM)) -> GFO)}.
...
-file("src/gleam/otp/supervisor.gleam", 87).
-spec start_child(child_spec(any(), GFV, GFW), GFV) -> {ok, child(GFW)} |
    {error, child_start_error()}.
...
-file("src/gleam/otp/supervisor.gleam", 106).
-spec shutdown_child(
    gleam@erlang@process:pid_(),
...
-file("src/gleam/otp/supervisor.gleam", 110).
-spec perform_instruction_for_child(
    GGJ,
    instruction(),
    child_spec(any(), GGJ, GGL),
    child(GGL)
) -> {ok, {child(GGL), instruction()}} | {error, child_start_error()}.
...
-file("src/gleam/otp/supervisor.gleam", 132).
-spec add_child_to_starter(
    starter(GGT),
    child_spec(any(), GGT, GGW),
    child(GGW)
) -> starter(GGW).
...
-file("src/gleam/otp/supervisor.gleam", 164).
-spec start_and_add_child(starter(GHC), child_spec(any(), GHC, GHF)) -> children(GHF).
...
-file("src/gleam/otp/supervisor.gleam", 206).
?DOC(
...
-spec supervisor(
    fun((GHS) -> {ok, gleam@erlang@process:subject(GHT)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GHT, GHS, GHS).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("src/gleam/otp/supervisor.gleam", 228).
?DOC(
...
-spec worker(
    fun((GIA) -> {ok, gleam@erlang@process:subject(GIB)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GIB, GIA, GIA).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("src/gleam/otp/supervisor.gleam", 241).
?DOC(
...
-spec returning(
    child_spec(GII, GIJ, any()),
    fun((GIJ, gleam@erlang@process:subject(GII)) -> GIP)
) -> child_spec(GII, GIJ, GIP).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), GIU)) -> gleam@otp@actor:init_result(state(GIU), message()).
init(Spec) ->
...
-file("src/gleam/otp/supervisor.gleam", 305).
-spec handle_exit(gleam@erlang@process:pid_(), state(GJA)) -> gleam@otp@actor:next(message(), state(GJA)).
...
-file("src/gleam/otp/supervisor.gleam", 345).
-spec loop(message(), state(GJF)) -> gleam@otp@actor:next(message(), state(GJF)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
...
-file("src/gleam/otp/supervisor.gleam", 377).
?DOC(
...
-file("src/gleam/otp/supervisor.gleam", 408).
?DOC(
