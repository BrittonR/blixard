...
-opaque set(EGE) :: {set, gleam@dict:dict(EGE, list(nil))}.
...
-spec contains(set(EGP), EGP) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(EGR), EGR) -> set(EGR).
delete(Set, Member) ->
...
-spec fold(set(EHA), EHC, fun((EHC, EHA) -> EHC)) -> EHC.
...
-spec filter(set(EHD), fun((EHD) -> boolean())) -> set(EHD).
...
-spec drop(set(EHK), list(EHK)) -> set(EHK).
drop(Set, Disallowed) ->
...
-spec take(set(EHO), list(EHO)) -> set(EHO).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(EHW), set(EHW)) -> {set(EHW), set(EHW)}.
order(First, Second) ->
...
-spec intersection(set(EIB), set(EIB)) -> set(EIB).
...
-spec difference(set(EIF), set(EIF)) -> set(EIF).
...
-spec is_subset(set(EIJ), set(EIJ)) -> boolean().
...
-spec is_disjoint(set(EIM), set(EIM)) -> boolean().
...
-spec each(set(EIT), fun((EIT) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(EGM), EGM) -> set(EGM).
insert(Set, Member) ->
...
-spec from_list(list(EGX)) -> set(EGX).
from_list(Members) ->
...
-spec map(set(EHG), fun((EHG) -> EHI)) -> set(EHI).
map(Set, Fun) ->
...
-spec union(set(EHS), set(EHS)) -> set(EHS).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(EIP), set(EIP)) -> set(EIP).
