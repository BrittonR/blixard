...
use blixard_vm::{
    ...
    process_manager::{VmProcessManager, CommandExecutor, RealCommandExecutor},
    types::{VmConfig, Hypervisor, NixModule},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let process_manager = Arc::new(VmProcessManager::new(
        runtime_dir.clone(),
        executor,
    ));
    ...
    println!("Generating both template types for: {}", comparison_config.name);
