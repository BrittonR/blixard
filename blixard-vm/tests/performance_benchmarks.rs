...
mod performance_benchmarks {
    ...
    async fn benchmark_concurrent_operations() {
        ...
        println!(
            "Concurrent creation of {} VMs took: {:?}",
            concurrent_vms, total_time
        );
        ...
        println!(
            "Average concurrent creation time: {:?}",
            avg_concurrent_time
        );
    ...
    async fn benchmark_memory_usage() {
        ...
        for i in 0..max_vms {
            ...
            if i > 0 && i % 10 == 0 {
                ...
                let early_avg: Duration =
                    creation_times[0..10.min(i)].iter().sum::<Duration>() / 10.min(i) as u32;

                println!(
                    "After {} VMs: recent avg {:?}, early avg {:?}",
                    i + 1,
                    recent_avg,
                    early_avg
                );
                ...
                assert!(
                    recent_avg < early_avg * 3,
                    "Significant performance degradation detected at {} VMs",
                    i + 1
                );
            }
        }
        ...
        println!(
            "Successfully created {} VMs with stable performance",
            max_vms
        );
    ...
    async fn benchmark_flake_generation() {
        ...
        for i in 0..iterations {
            let config = blixard_vm::types::VmConfig {
                ...
                vcpus: (i % 4) + 1,
                memory: ((i % 4) + 1) * 512,
                networks: vec![blixard_vm::types::NetworkConfig::Routed {
                    id: format!("vm-bench-{}", i),
                    mac: format!("02:00:00:00:{:02x}:{:02x}", i / 256, i % 256),
...
mod stress_tests {
    ...
    async fn stress_rapid_cycles() {
        ...
        assert!(
            avg_cycle_time < Duration::from_millis(200),
            "Rapid cycles too slow: {:?}",
            avg_cycle_time
        );
