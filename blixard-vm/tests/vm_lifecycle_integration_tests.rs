...
mod lifecycle_tests {
    ...
    async fn test_multiple_vm_lifecycle() {
        ...
        for (i, config) in vm_configs.iter().enumerate() {
            ...
            let flake_path = config_dir.join("vms").join(&config.name).join("flake.nix");
            assert!(!flake_path.exists(), "Deleted VM flake should be cleaned up");
        }
    ...
    async fn build_vm_nix(flake_path: &PathBuf, vm_name: &str) -> Result<PathBuf, String> {
        ...
        let target = format!(".#nixosConfigurations.{}.config.microvm.runner.qemu", vm_name);
...
mod performance_tests {
    ...
    async fn test_vm_creation_performance() {
        ...
        assert!(creation_time < Duration::from_secs(1), 
            "VM creation should be fast, took {:?}", creation_time);
    ...
    async fn test_concurrent_vm_operations() {
        ...
        println!("Created {} VMs in {:?} (avg: {:?} per VM)", 
            num_vms, total_time, total_time / num_vms);
