use blixard_core::{types::VmConfig as CoreVmConfig, vm_backend::VmBackend};
...
mod flake_validation_tests {
    ...
    async fn test_generated_flake_validation() {
        ...
        match run_nix_flake_check(&flake_dir).await {
            ...
            Err(e) => {
                println!(
                    "⚠ Nix flake check failed (expected in environments without Nix): {}",
                    e
                );
...
mod template_tests {
    ...
    async fn test_template_syntax() {
        ...
        assert!(
            ...
            "Template should contain Tera variables"
        );
        assert!(
            template_content.contains("inputs"),
            "Template should have inputs"
        );
        assert!(
            template_content.contains("outputs"),
            "Template should have outputs"
        );
        assert!(
            template_content.contains("nixosConfigurations"),
            "Template should have nixosConfigurations"
        );
