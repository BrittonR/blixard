...
mod error_scenario_tests {
    ...
    async fn test_filesystem_permission_errors() {
        ...
        let database = Arc::new(Database::create(&db_path).unwrap());
        match MicrovmBackend::new(config_dir, data_dir, database) {
            Ok(backend) => {
    ...
    async fn test_corrupted_config_recovery() {
        ...
        assert!(
            !vm_dir.exists(),
            "Corrupted VM directory should be cleaned up"
        );
    }
    ...
    async fn test_template_generation_errors() {
        ...
        assert!(
            result.is_err(),
            "Should fail with missing template directory"
        );
...
mod recovery_tests {
    ...
    async fn test_backend_recovery() {
...

        {
            ...
            let database = Arc::new(Database::create(&db_path).unwrap());
            let backend1 = MicrovmBackend::new(config_dir.clone(), data_dir.clone(), database).unwrap();
        ...
        assert!(
            result.is_ok(),
            "Should be able to recreate VM after corruption"
        );
