...
fn render_p2p_status_bar(f: &mut Frame, area: Rect, app: &App) {
    ...
    let status_text = if app.p2p_enabled {
        ...
        vec![
            ...
            Span::styled(
                " - Press 'p' to enable P2P networking",
                Style::default().fg(Color::Gray),
            ),
        ]
    };

    let status_bar = Paragraph::new(Line::from(status_text)).block(
        Block::default()
            .borders(Borders::ALL)
...
fn render_transfer_activity(f: &mut Frame, area: Rect, app: &App) {
    ...
    let transfers: Vec<ListItem> = app
        ...
        .map(|transfer| {
            let progress =
                (transfer.bytes_transferred as f64 / transfer.total_bytes as f64 * 100.0) as u16;
            let speed = format_bytes_per_sec(transfer.speed_bps);

            let icon = if transfer.is_upload {
                "⬆️"
            } else {
                "⬇️"
            };
            let direction = if transfer.is_upload {
                "Uploading"
            } else {
                "Downloading"
            };
            ...
            ListItem::new(text).style(Style::default().fg(if transfer.is_upload {
                INFO_COLOR
            } else {
                SUCCESS_COLOR
            }))
    ...
    let list = List::new(transfers).block(
        Block::default()
            .borders(Borders::ALL)
