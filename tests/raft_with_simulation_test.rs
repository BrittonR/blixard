...
async fn test_three_node_election_timing() {
    ...
    for (node_id, timeout_ms) in election_timeouts.iter().enumerate() {
        ...
        let handle = madsim::task::spawn(async move {
            println!("Node {} waiting for election timeout: {}ms", node_id + 1, timeout);
...
async fn test_network_partition_timing() {
    ...
    assert!(first_three.iter().all(|&id| id <= 3), "First partition should communicate first");
    assert!(last_two.iter().all(|&id| id >= 4), "Second partition should communicate later");
...
async fn test_deterministic_leader_election() {
    ...
    assert!(election_results.windows(2).all(|w| w[0] == w[1]), 
