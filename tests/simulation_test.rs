...
async fn test_network_partition_recovery() {
    ...
    for node_id in 1..=5 {
        ...
        let handle = madsim::task::spawn(async move {
            ...
            println!(
                "Node {} starting with partition delay {}ms",
                node_id, partition_delay
            );
            ...
            for i in 0..3 {
                ...
                state
                    .lock()
                    .await
                    .push(format!("Node {} - operation {}", node_id, i));
            }
        });
        handles.push(handle);
    }
...
async fn test_crash_recovery_simulation() {
...

    {
            ...
            let handle = madsim::task::spawn(async move {
                ...
                for i in 0..3 {
                    ...
                    state
                        .lock()
                        .await
                        .push(format!("Node {} - data {}", node_id, i));
                }
            });
            handles.push(handle);
        }
        ...
        println!(
            "Data written before crash: {} entries",
            persistent_state.lock().await.len()
        );
    }
    ...
    // Phase 2: Restart and verify data
    {
        ...
        println!(
            "Recovered {} data entries after crash",
            recovered_data.len()
        );
