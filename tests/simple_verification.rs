...
async fn verify_madsim_is_working() {
    ...
    assert!(
        after_sleep >= Duration::from_secs(5),
        "Time should advance at least 5 seconds!"
    );
    ...
    assert!(
        total_elapsed >= Duration::from_secs(7),
        "Total should be at least 7 seconds!"
    );
...
async fn verify_network_simulation() {
    ...
    let server_handle = madsim::task::spawn(async move {
        ...
        messages_clone
            .lock()
            .await
            .push("Server received message".to_string());
        sleep(Duration::from_millis(50)).await;
        messages_clone
            .lock()
            .await
            .push("Server sent response".to_string());
    });
    ...
    let client_handle = madsim::task::spawn(async move {
        ...
        messages_clone
            .lock()
            .await
            .push("Client sending message".to_string());
        sleep(Duration::from_millis(100)).await;
        messages_clone
            .lock()
            .await
            .push("Client received response".to_string());
    });
...
async fn verify_time_precision() {
    ...
    for duration in times {
        ...
        assert!(
            ...
            "Sleep should not have too much overhead"
        );
        println!(
            "✅ Sleep {:?} = elapsed {:?} (exact match!)",
            duration, elapsed
        );
    }
