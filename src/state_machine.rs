...
impl StateMachine {
    ...
    pub async fn apply(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        let command: StateMachineCommand =
            bincode::deserialize(data).context("Failed to deserialize command")?;

        debug!("Applying command: {:?}", command);

        match command {
            StateMachineCommand::CreateService { info } => self.apply_create_service(info).await,
            StateMachineCommand::UpdateService { name, info } => {
                self.apply_update_service(name, info).await
            }
            StateMachineCommand::DeleteService { name } => self.apply_delete_service(name).await,
            ...
            StateMachineCommand::RemoveNode { id } => self.apply_remove_node(id).await,
            StateMachineCommand::CreateVm { vm } => self.apply_create_vm(vm).await,
            StateMachineCommand::UpdateVmStatus { name, status } => {
                self.apply_update_vm_status(name, status).await
            }
            StateMachineCommand::DeleteVm { name } => self.apply_delete_vm(name).await,
            StateMachineCommand::ClusterEvent { event } => self.apply_cluster_event(event).await,
    ...
    pub async fn snapshot(&self) -> Result<Vec<u8>> {
        ...
        bincode::serialize(&snapshot).context("Failed to serialize snapshot")
    }
