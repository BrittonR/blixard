...
use crate::state_machine::{ClusterEvent, NodeInfo as StateMachineNodeInfo, ServiceInfo};
use crate::types::*;
...
impl Storage {
    ...
    pub fn get_raft_log(&self, index: u64) -> Result<Option<Vec<u8>>> {
        ...
        let result = table.get(&index)?.map(|data| data.value().to_vec());
        Ok(result)
    }
    ...
    pub fn get_raft_state(&self, key: &str) -> Result<Option<Vec<u8>>> {
        ...
        let result = table.get(key)?.map(|data| data.value().to_vec());
        Ok(result)
    }
    ...
    pub fn clear_all(&self) -> Result<()> {
        let write_txn = self.db.begin_write()?;
        {
            ...
            let vm_keys: Vec<String> = vm_table
                .iter()?
                .map(|r| r.map(|(k, _)| k.value().to_string()))
                .collect::<Result<Vec<_>, _>>()?;
            let node_keys: Vec<u64> = node_table
                .iter()?
                .map(|r| r.map(|(k, _)| k.value()))
                .collect::<Result<Vec<_>, _>>()?;
            let service_keys: Vec<String> = service_table
                .iter()?
                .map(|r| r.map(|(k, _)| k.value().to_string()))
                .collect::<Result<Vec<_>, _>>()?;
