...
use crate::network_v2::RaftNetwork;
use crate::runtime_traits::{Runtime, Clock};

/// Raft node implementation with persistent storage and state machine
/// Now generic over the Runtime to support both real and simulated execution
...
impl<R: Runtime + 'static> RaftNode<R> {
    pub async fn new(
        node_id: u64, 
        ...
        if !peers.is_empty() {
            ...
            info!("Node {} bootstrapped with voters: {:?}", node_id, conf_state.voters);
        } else {
            // Single node mode
            let mut conf_state = ConfState::default();
        ...
        let network = Arc::new(RaftNetwork::new(node_id, bind_addr, message_tx.clone(), runtime.clone()));
        
        // Start network listener in background using runtime
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        // Use runtime interval instead of tokio directly
        let runtime = self.runtime.clone();
        let mut ticker = runtime.clock().interval(Duration::from_millis(100));
    ...
    async fn handle_proposal(&mut self, command: StateMachineCommand) -> Result<()> {
        #[cfg(feature = "failpoints")]
        fail_point!("raft::before_proposal", |_| Err(anyhow::anyhow!("Injected proposal failure")));
    ...
    async fn handle_message(&mut self, message: Message) -> Result<()> {
        ...
        #[cfg(feature = "failpoints")]
        fail_point!("raft::handle_message", |_| Err(anyhow::anyhow!("Injected message handling failure")));
    ...
    async fn process_ready(&mut self) -> Result<()> {
        ...
        if messages.is_empty() {
            ...
            info!("Node {} ready has {} messages to send", self.node_id, messages.len());
            for msg in messages {
                info!("Node {} sending message to node {}: {:?}", self.node_id, msg.to, msg.msg_type);
        ...
        if !persisted_messages.is_empty() {
            info!("Node {} has {} persisted messages to send", self.node_id, persisted_messages.len());
            for msg in persisted_messages {
                info!("Node {} sending persisted message to node {}: {:?}", self.node_id, msg.to, msg.msg_type);
                self.send_message(msg).await?;
            }
        }
        ...
        if !light_messages.is_empty() {
            info!("Node {} has {} light messages to send", self.node_id, light_messages.len());
            for msg in light_messages {
                info!("Node {} sending light message to node {}: {:?}", self.node_id, msg.to, msg.msg_type);
                self.send_message(msg).await?;
            }
        }
        ...
        if !light_ready.committed_entries().is_empty() {
            self.apply_entries(light_ready.take_committed_entries()).await?;
        }
    ...
    async fn apply_entries(&mut self, entries: Vec<Entry>) -> Result<()> {
        for entry in entries {
            ...
            debug!("Applying entry at index {} type {:?}", entry.index, entry.entry_type);
    ...
    async fn send_message(&self, msg: Message) -> Result<()> {
        #[cfg(feature = "failpoints")]
        fail_point!("raft::before_send_message", |_| Err(anyhow::anyhow!("Injected send failure")));
...
mod tests {
    ...
    #[tokio::test]
    async fn test_raft_node_creation_with_simulated_runtime() {
        ...
        let node = RaftNode::new(1, bind_addr, storage, vec![], runtime).await.unwrap();
