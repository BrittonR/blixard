...
impl RaftNetwork {
    pub fn new(node_id: u64, bind_addr: SocketAddr, incoming_tx: mpsc::Sender<Message>) -> Self {
        Self {
            node_id,
            bind_addr,
    ...
    pub async fn send_message(&self, msg: Message) -> Result<()> {
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(1), connect_future).await {
            ...
            Ok(Err(e)) => {
                debug!(
                    "Failed to connect to {}: {} (node may not be ready)",
                    addr, e
                );
...
fn deserialize_message(data: &[u8]) -> Result<Message> {
    if data.len() < 53 {
        // Minimum message size (adjusted for i32 msg_type)
