...
async fn test_leader_election_with_network_failures() {
    ...
    assert!(result.is_ok(), "Failed to elect new leader with network failures");
...
async fn test_storage_failures_during_replication() {
    ...
    assert!(created > 0, "No VMs were created despite only 20% failure rate");
    assert!(failed > 0, "No failures detected, failpoint may not be working");
    ...
    assert!(vm_counts.windows(2).all(|w| w[0] == w[1]),
        "Nodes have inconsistent VM counts: {:?}", vm_counts);
}
...
async fn test_chaos_engineering_scenario() {
    ...
    assert!(successes > 25, "Too few successes under chaos: {}", successes);
    ...
    if running_counts.len() > 1 {
        ...
        assert!(
            running_counts.iter().all(|&c| (c as i32 - first as i32).abs() <= 5),
            "Nodes diverged too much under chaos: {:?}", running_counts
        );
    }
}
...
async fn test_cascading_failure_recovery() {
    ...
    for node in cluster.get_nodes() {
        let vms = node.list_vms().await.unwrap();
        assert_eq!(vms.len(), 5, "Node {} lost data during cascading failure", node.id());
    }
}