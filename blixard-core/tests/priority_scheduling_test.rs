...
use blixard_core::types::{VmConfig, VmStatus, VmState};
use blixard_core::raft_manager::WorkerCapabilities;
use blixard_core::storage::{WORKER_TABLE, WORKER_STATUS_TABLE, VM_STATE_TABLE};
use redb::Database;
use std::sync::Arc;
...
async fn test_priority_based_placement_without_preemption() {
    ...
    // Register a worker node
    {
        let write_txn = db.begin_write().unwrap();
        {
            ...
            // Mark as online
            status_table.insert(node_id.to_le_bytes().as_slice(), [0u8].as_slice()).unwrap();
        }
        write_txn.commit().unwrap();
    }
    ...
    let decision1 = scheduler.schedule_vm_placement(&high_priority_vm, strategy.clone()).await.unwrap();
    assert_eq!(decision1.selected_node_id, 1);
    assert!(decision1.preemption_candidates.is_empty());
    
    let decision2 = scheduler.schedule_vm_placement(&low_priority_vm, strategy).await.unwrap();
...
async fn test_priority_based_placement_with_preemption() {
    ...
    // Register a worker node with limited resources
    {
        ...
        let existing_vm2 = VmState {
            ...
            vm_table.insert("existing-low-1", bincode::serialize(&existing_vm1).unwrap().as_slice()).unwrap();
            vm_table.insert("existing-low-2", bincode::serialize(&existing_vm2).unwrap().as_slice()).unwrap();
        }
        write_txn.commit().unwrap();
    }
    ...
    let decision = scheduler.schedule_vm_placement(&high_priority_vm, strategy).await.unwrap();
    ...
    assert!(decision.preemption_candidates.iter().any(|c| c.vm_name == "existing-low-2"));
...
async fn test_preemption_policy_enforcement() {
    ...
    let cost_aware_policy = PreemptionPolicy::CostAware { max_cost_increase: 10.0 };
...
async fn test_preemption_execution() {
    ...
    let preempted = scheduler.execute_preemption(&candidates, &config).await.unwrap();
    ...
    let forced_preempted = scheduler.execute_forced_preemption(&candidates).await.unwrap();
