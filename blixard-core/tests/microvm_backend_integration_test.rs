...
async fn test_microvm_backend_registration() -> BlixardResult<()> {
    ...
    assert!(backends.contains(&"microvm"), "Microvm backend should be available");
...
async fn test_microvm_create_simple_vm() -> BlixardResult<()> {
    ...
    assert!(create_response.into_inner().success, "Failed to create microvm");
    ...
    assert!(delete_response.into_inner().success, "Failed to delete microvm");
...
async fn test_microvm_with_networking() -> BlixardResult<()> {
    ...
    println!("VM started successfully - network connectivity test skipped (get_vm_ip not implemented)");
