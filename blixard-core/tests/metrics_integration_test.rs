...
async fn test_metrics_recording() {
    ...
    // Record some metrics
    metrics.raft_proposals_total.add(5, &[attributes::node_id(1)]);
    metrics.raft_proposals_failed.add(2, &[attributes::node_id(1)]);
...
fn test_metrics_attributes() {
    ...
    assert_eq!(format!("{:?}", node_attr.value), format!("{:?}", opentelemetry::Value::I64(42)));
