...
pub enum Response {
    ...
    VmStatus { exists: bool, state: Option<String> },
        ...
        match operation {
            Operation::CreateVm { name, config } => {
                if self.vms.contains_key(name) {
                    ...
                    self.vms.insert(name.clone(), VmState {
                        config: config.clone(),
                        running: false,
                    });
            ...
            _ => Response::Error { message: "Unsupported operation".to_string() },
        }
    }
}
...
mod tests {
    ...
    async fn test_linearizable_history() {
        ...
        let id1 = history.begin_operation(Operation::CreateVm {
            name: "vm1".to_string(),
            config: VmConfig { memory_mb: 1024, vcpus: 2 },
        });
    ...
    async fn test_non_linearizable_history() {
        ...
        let id2 = history.begin_operation(Operation::CreateVm {
            name: "vm1".to_string(),
            config: VmConfig { memory_mb: 2048, vcpus: 4 },
        });
