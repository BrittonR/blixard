...
impl DeterministicExecutor {
    ...
    async fn execute_event(&mut self, event: DeterministicEvent) -> BlixardResult<()> {
        ...
        match event {
            ...
            DeterministicEvent::ClientProposal { node_id, data, proposal_id } => {
                if let Some(cluster) = &self.cluster {
            ...
            DeterministicEvent::NetworkEvent { event_type, .. } => {
                match event_type {
                    ...
                    NetworkEventType::AddLatency { from, to, latency_ms } => {
                        self.network.latencies.insert((from, to), latency_ms);
                    }
                    NetworkEventType::AddPacketLoss { from, to, loss_rate } => {
            ...
            DeterministicEvent::InjectMessageLoss { from, to, drop_probability, .. } => {
    ...
    async fn check_invariant(&self, invariant: &InvariantType, cluster: &TestCluster) -> Option<String> {
        match invariant {
                ...
                for (id, node) in cluster.nodes() {
                    if node.shared_state.is_leader().await {
                        if let Ok(status) = node.shared_state.get_raft_status().await {
                            term_leaders.entry(status.current_term)
                                .or_default()
                                .push(*id);
                        }
                    }
...
async fn replay_trace(trace_path: &Path) -> BlixardResult<ExecutionTrace> {
    let file = File::open(trace_path)
        .map_err(|e| BlixardError::Io {
            operation: "open trace file".to_string(),
            path: trace_path.to_path_buf(),
            source: e,
        })?;
    
    let trace: ExecutionTrace = serde_json::from_reader(file)
    ...
    RUNTIME.block_on(async {
        ...
        for seed in 0..100 {
            ...
            if !executor.trace.violations.is_empty() {
                ...
                println!("Found bug with seed {}: {:?}", seed, executor.trace.violations);
