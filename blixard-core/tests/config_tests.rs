...
fn test_config_builder() {
    ...
    assert_eq!(
        config.cluster.join_address,
        Some("192.168.1.1:8000".to_string())
    );
...
"#;
    ...
    assert_eq!(
        config.cluster.raft.conf_change_timeout,
        Duration::from_secs(10)
    );

    assert_eq!(config.cluster.peer.max_connections, 200);
    assert_eq!(
        config.cluster.peer.connection_timeout,
...
fn test_hot_reloadable_detection() {
    ...
    assert!(Config::is_hot_reloadable(
...
"#;
    ...
    assert_eq!(
        config.cluster.raft.proposal_timeout,
        Duration::from_secs(90)
    );
    assert_eq!(
        config.cluster.raft.restart_delay,
    ...
    assert_eq!(
        config.cluster.peer.reconnect_delay,
        Duration::from_millis(2500)
    );
    assert_eq!(
        config.cluster.peer.health_check_interval,
...
"#;
    ...
    assert!(config.security.tls.enabled);
    assert_eq!(
        config.security.tls.cert_file,
        Some(PathBuf::from("/etc/blixard/certs/server.crt"))
    );
    assert_eq!(
        config.security.tls.key_file,
        Some(PathBuf::from("/etc/blixard/certs/server.key"))
    );
    assert_eq!(
        config.security.tls.ca_file,
        Some(PathBuf::from("/etc/blixard/certs/ca.crt"))
    );
    assert!(config.security.tls.require_client_cert);
    ...
    assert_eq!(
        config.security.auth.token_file,
        Some(PathBuf::from("/etc/blixard/tokens/auth.token"))
    );
}
...
fn test_security_config_builder() {
    ...
    assert_eq!(
        config.security.tls.key_file,
        Some(PathBuf::from("/custom/key.pem"))
    );
...
fn test_security_environment_overrides() {
    ...
    assert_eq!(
        config.security.auth.token_file,
        Some(PathBuf::from("/env/tokens.json"))
    );
