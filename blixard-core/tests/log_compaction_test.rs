...
async fn test_log_compaction_basic() {
    ...
    timing::wait_for_condition_with_backoff(
        || async {
            cluster.get_node(1).unwrap().shared_state.is_leader().await
        },
    ...
    for i in 0..1200 {
        ...
        node.node.submit_task(&task_id, task_spec).await
    ...
    let db = node.shared_state.get_database().await.expect("Should have database");
    let storage = RedbRaftStorage { database: db.clone() };
    ...
    assert!(log_entries < 1200, 
        "Log should be compacted, but has {} entries", log_entries);
}
...
async fn test_recovery_after_compaction() {
    ...
    for i in 0..1500 {
        ...
        cluster.get_node(1).unwrap()
            .node.submit_task(&task_id, task_spec).await
            .expect("Should submit task");
    ...
    let status_before = cluster.get_node(1).unwrap()
        .node.get_task_status(sample_task_id).await
    ...
    timing::wait_for_condition_with_backoff(
        || async {
            new_cluster.get_node(1).unwrap().shared_state.is_leader().await
        },
        Duration::from_secs(10),
        Duration::from_millis(200),
    ).await.expect("Node should recover and become leader");
    
    // Verify some tasks were recovered
    let status_after = new_cluster.get_node(1).unwrap()
        .node.get_task_status(sample_task_id).await
        .expect("Should get task status");
    assert!(status_after.is_some(), "Task should be recovered after restart");
    ...
    let new_task_status = new_cluster.get_node(1).unwrap()
        .node.get_task_status("recovery-task-1550").await
        .expect("Should get new task status");
    assert!(new_task_status.is_some(), "New tasks should complete after recovery");
}
...
async fn test_compaction_multi_node() {
    ...
    for i in 0..1100 {
        ...
        leader_node.node.submit_task(&task_id, task_spec).await
            .expect("Should submit task");
    ...
    for i in (0..1100).step_by(100) {
        ...
        for node_id in 1..=3 {
            if let Ok(node) = cluster.get_node(node_id) {
                ...
                assert!(status.is_ok() && status.unwrap().is_some(), 
                    "Task {} should exist on node {}", task_id, node_id);
            }
        }
    }
}
