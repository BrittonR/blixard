...
use blixard_core::{
    error::BlixardResult,
    iroh_transport_v2::{IrohTransportV2, DocumentType},
...
async fn test_p2p_document_sync() -> BlixardResult<()> {
    ...
    transport.create_or_join_doc(DocumentType::ClusterConfig).await?;
    transport.create_or_join_doc(DocumentType::Metadata).await?;
    transport.create_or_join_doc(DocumentType::NodeState).await?;
    
    // Write state to different documents
    let cluster_config = b"cluster_version: 1.0";
    transport.write_to_doc(DocumentType::ClusterConfig, "version", cluster_config).await?;
    
    let node_state = b"status: healthy";
    transport.write_to_doc(DocumentType::NodeState, "node-1", node_state).await?;
    
    // Read back the state
    let read_config = transport.read_from_doc(DocumentType::ClusterConfig, "version").await?;
    assert_eq!(read_config, cluster_config);
    
    let read_state = transport.read_from_doc(DocumentType::NodeState, "node-1").await?;
