...
async fn test_rapid_sequential_joins() {
    ...
    for (node_num, result, duration) in results {
        match result {
            Ok(node_id) => {
                println!(
                    "✓ Node {} (id {}) joined successfully in {:?}",
                    node_num, node_id, duration
                );
                successes += 1;
            }
            Err(e) => {
                println!(
                    "✗ Node {} failed to join in {:?}: {:?}",
                    node_num, duration, e
                );
    ...
    println!(
        "\nRapid join results: {} successes, {} failures",
        successes,
        failures.len()
    );
    ...
    let (leader_id, node_ids, _term) = node1
        .shared_state
        .get_cluster_status()
        .await
    ...
    println!(
        "Final cluster state - Leader: {}, Total nodes: {}, Node IDs: {:?}",
        leader_id,
        node_ids.len(),
        node_ids
    );
...
async fn test_join_during_high_load() {
    ...
    match &join_result {
        Ok(node_id) => {
            println!(
                "✓ Node {} joined successfully during load in {:?}",
                node_id, join_duration
            );
        }
        Err(e) => {
            println!(
                "✗ Failed to join during load after {:?}: {:?}",
                join_duration, e
            );
        }
    }
    ...
    let (final_leader_id, node_ids, _term) = leader
        .shared_state
        .get_cluster_status()
        .await
        .expect("Should get cluster status");
    ...
    println!(
        "Final cluster state - Leader: {}, Nodes: {:?}",
        final_leader_id, node_ids
    );

    // The join should have succeeded
    assert!(
        join_result.is_ok(),
        "Node should be able to join during load"
    );
...
async fn test_leader_change_during_join() {
    ...
    // Find new leader
    let new_leader = cluster.nodes().values().find(|n| {
        let status = futures::executor::block_on(n.shared_state.get_raft_status());
        status.map(|s| s.is_leader).unwrap_or(false)
    });
...
async fn test_join_retry_after_failure() {
    ...
    while attempts < max_attempts {
        ...
        match cluster.add_node().await {
            Ok(node_id) => {
                println!(
                    "✓ Node {} joined successfully on attempt {}",
                    node_id, attempts
                );
    ...
    assert!(
        final_result.unwrap().is_ok(),
        "Join should succeed within {} attempts",
        max_attempts
    );
