...
async fn test_p2p_manager_basic_operations() {
    ...
    let p2p_manager = Arc::new(P2pManager::new(1, &node_dir, config).await.unwrap());
    ...
    let hash = p2p_manager
        .share_data(test_data.to_vec(), "test-data")
        .await
        .unwrap();
...
async fn test_p2p_manager_multi_node() {
    ...
    let p2p_manager1 = Arc::new(P2pManager::new(1, &node1_dir, config1).await.unwrap());
    let p2p_manager2 = Arc::new(P2pManager::new(2, &node2_dir, config2).await.unwrap());
    ...
    match p2p_manager1.connect_p2p_peer(2, &addr2).await {
        ...
        Err(e) => {
            info!(
                "P2P peer connection failed (expected in test environment): {}",
                e
            );
        }
    }
...
async fn test_p2p_with_discovery() {
    ...
    let p2p_manager = Arc::new(
        P2pManager::new_with_discovery(1, &node_dir, config, Some(discovery_manager))
            .await
            .unwrap(),
    );
    ...
    p2p_manager
        .store_metadata("discovery-test", b"test-value")
        .await
        .unwrap();
