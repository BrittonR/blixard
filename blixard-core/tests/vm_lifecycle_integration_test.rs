...
use blixard_core::{error::BlixardResult, iroh_types, test_helpers::TestNode};
...
async fn test_single_node_vm_create_start_stop_delete() -> BlixardResult<()> {
    ...
    println!(
        "Raft status after initialization: is_leader={}, leader_id={:?}",
                ...
                Ok(vm_info) => {
                    if let Some(info) = vm_info {
                        ...
                        if info.state == 3 {
                            ...
                            println!(
                                "VM is in Starting state, waiting for transition to Running..."
                            );
                        }
                    } else {
...
async fn test_three_node_cluster_vm_scheduling() -> BlixardResult<()> {
    ...
    assert!(
        create_response1.into_inner().success,
        "Failed to create VM 1"
    );

    let create_response2 = client1.create_vm(vm2_config).await?;
    assert!(
        create_response2.into_inner().success,
        "Failed to create VM 2"
    );

    let create_response3 = client1.create_vm(vm3_config).await?;
    assert!(
        create_response3.into_inner().success,
        "Failed to create VM 3"
    );
    ...
    let start1 = client1
        .start_vm(iroh_types::StartVmRequest {
            name: "test-vm-1".to_string(),
        })
        .await?;
    ...
    let delete1 = client1
        .delete_vm(iroh_types::DeleteVmRequest {
            name: "test-vm-1".to_string(),
        })
        .await?;
