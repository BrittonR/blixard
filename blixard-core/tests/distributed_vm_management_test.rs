...
async fn test_distributed_vm_lifecycle() {
    ...
    node.shared_state.create_vm_through_raft(command)
        .await
    ...
    node.shared_state.send_vm_operation_through_raft(command)
        .await
    ...
    for node_id in 1..=3 {
        let test_node = cluster.get_node(node_id).unwrap_or_else(|_| panic!("Failed to get node {}", node_id));
        let vm_manager = test_node.shared_state.get_vm_manager()
            .await
            .expect("VM manager not initialized");
        
        let status = vm_manager.get_vm_status(&vm_config.name)
            .await
            .expect("Failed to get VM status");
        
        assert!(status.is_some(), "VM should exist on node {}", node_id);
        let (config, vm_status) = status.unwrap();
        assert_eq!(config.name, vm_config.name, "VM name should match on node {}", node_id);
        
        // The VM should be running or at least starting
        assert!(
            vm_status == VmStatus::Starting || vm_status == VmStatus::Running,
            "VM should be starting or running on node {}, but was {:?}", node_id, vm_status
        );
    }
...
async fn test_distributed_vm_scheduling() {
    ...
    let node = cluster.get_node(1).expect("Failed to get node 1");
    let vm_manager = node.shared_state.get_vm_manager()
        .await
        .expect("VM manager not initialized");
    ...
    for i in 1..=5 {
        ...
        ).await.expect("Failed to schedule VM");
    ...
    assert_eq!(vms_per_node.len(), 3, "VMs should be distributed across all 3 nodes");
...
async fn test_concurrent_vm_operations() {
    ...
    for i in 1..=3 {
        let node = cluster.get_node(i).unwrap_or_else(|_| panic!("Failed to get node {}", i));
    ...
    for i in 1..=3 {
        ...
        let status = vm_manager.get_vm_status(&vm_name)
            .await
            .expect("Failed to get VM status");
