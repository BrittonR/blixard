...
async fn test_three_node_cluster_join() {
    ...
    println!("Created node 2 at {} and joined via {}", node2.addr, node1.addr);
    ...
    println!("Created node 3 at {} and joined via {}", node3.addr, node1.addr);
    ...
    let voters3 = node3.shared_state.get_current_voters().await
    ...
    assert_eq!(sorted_voters1, vec![1, 2, 3], "Node 1 should see voters [1, 2, 3]");
    assert_eq!(sorted_voters2, vec![1, 2, 3], "Node 2 should see voters [1, 2, 3]");
    assert_eq!(sorted_voters3, vec![1, 2, 3], "Node 3 should see voters [1, 2, 3]");
    ...
    let raft_status3 = node3.shared_state.get_raft_status().await
    ...
    println!("Node 3 Raft status: is_leader={}, leader_id={:?}, term={}", 
