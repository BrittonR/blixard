...
async fn test_locality_aware_placement_strategy() {
    ...
    // Register workers in different datacenters
    {
        let write_txn = db.begin_write().unwrap();
        {
            ...
            for (node_id, dc, zone, cpu, mem) in [
                ...
                let worker_data = bincode::serialize(&(format!("node{}", node_id), capabilities)).unwrap();
                ...
                topology_table.insert(node_id.to_le_bytes().as_slice(), bincode::serialize(&topology).unwrap().as_slice()).unwrap();
            }
        }
        write_txn.commit().unwrap();
    }
    ...
    let decision = scheduler.schedule_vm_placement(&vm_config, strategy).await.unwrap();
    ...
    let decision2 = scheduler.schedule_vm_placement(&vm_config2, strategy2).await.unwrap();
    ...
    let result3 = scheduler.schedule_vm_placement(&vm_config2, strategy3).await;
    assert!(result3.is_err());
    assert!(result3.unwrap_err().to_string().contains("Strict locality requirement not met"));
}
...
async fn test_spread_across_failure_domains_strategy() {
        ...
        let write_txn = db.begin_write().unwrap();
        {
            ...
            for (node_id, zone, existing_vms) in [
                ...
                for i in 0..existing_vms {
                    ...
                    vm_table.insert(&vm_state.name, bincode::serialize(&vm_state).unwrap().as_slice()).unwrap();
                }
            }
        }
        write_txn.commit().unwrap();
    ...
    let decision2 = scheduler.schedule_vm_placement(&vm_config, strategy2).await.unwrap();
