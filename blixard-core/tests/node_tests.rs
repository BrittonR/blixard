...
use blixard_core::{
    error::BlixardError,
    node::Node,
    storage::{TASK_TABLE, VM_STATE_TABLE, WORKER_STATUS_TABLE, WORKER_TABLE},
    test_helpers::timing,
...
async fn test_node_lifecycle_with_initialization() {
    ...
    common::wait_for_condition(|| async { node.is_running().await }, Duration::from_secs(5))
        .await
        .expect("Node should start within timeout");
...
async fn test_cluster_operations_uninitialized() {
    ...
    assert!(
        result.is_err(),
        "Expected error for join_cluster on uninitialized node"
    );
    ...
    assert!(
        result.is_err(),
        "Expected error for leave_cluster on uninitialized node"
    );
    ...
    assert!(
        result.is_err(),
        "Expected error for get_cluster_status on uninitialized node, got {:?}",
        result
    );
}
...
async fn test_database_persistence_across_restarts() {
    ...
    // Create and initialize first node
    {
        ...
        .await
        .expect("Node should stop within timeout");
    ...
    .await
    .expect("Database file should be released within timeout");
...
async fn test_database_concurrent_access() {
    ...
    .await
    .expect("All VMs should be created within timeout");
...
async fn test_raft_manager_initialization() {
    ...
    let result = node
        .send_raft_message(2, raft::prelude::Message::default())
        .await;
    assert!(matches!(result, Err(BlixardError::Internal { .. })));
    ...
    common::wait_for_condition(
        || async {
            ...
            node.send_raft_message(2, raft::prelude::Message::default())
                .await
                .is_ok()
        },
    ...
    .await
    .expect("Raft should be ready within timeout");
...
async fn test_task_status_retrieval() {
    ...
    let status_response = leader_client
...
async fn test_bootstrap_mode_registration() {
    ...
    assert!(
        worker_data.is_some(),
        "Worker should be registered in database"
    );
}
...
async fn test_leave_cluster() {
    ...
    let status_before = leader_client
        .get_cluster_status(ClusterStatusRequest {})
        .await
        .expect("Should get cluster status")
        .into_inner();
    assert_eq!(
        status_before.nodes.len(),
        2,
        "Should have 2 nodes before leaving"
    );

    // Node 2 leaves the cluster (request must go through leader)
    let leave_response = leader_client
        .leave_cluster(LeaveRequest { node_id: 2 })
        .await
    ...
    .await
    .expect("Cluster should have 1 node after leaving");

    // Final verification
    let status_after = leader_client
        .get_cluster_status(ClusterStatusRequest {})
        .await
        .expect("Should get cluster status")
        .into_inner();
    assert_eq!(
        status_after.nodes.len(),
        1,
        "Should have 1 node after leaving"
    );
...
async fn test_vm_command_channel_closed() {
    ...
    let result = node
        .send_vm_command(VmCommand::UpdateStatus {
            name: "test".to_string(),
            status: VmStatus::Running,
        })
        .await;
    ...
    assert!(
        result.is_err(),
        "Expected error after stop clears VM manager"
    );
}
...
async fn test_concurrent_start_stop() {
    ...
    let handle1 = tokio::spawn(async move { node_shared.clone() });
...
async fn test_vm_lifecycle_operations() {
    use blixard_core::iroh_types::{
        CreateVmRequest, GetVmStatusRequest, ListVmsRequest, StartVmRequest, StopVmRequest,
    };
    use blixard_core::test_helpers::TestCluster;
    ...
    .await
    .expect("VM should be created within timeout");
    ...
    assert!(
        start_response.into_inner().success,
    ...
    let final_status_resp = client
        .get_vm_status(final_status_req)
        .await
