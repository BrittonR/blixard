...
async fn create_test_quota_manager() -> (QuotaManager, TempDir) {
    ...
    let quota_manager = QuotaManager::new(storage)
        .await
        .expect("Failed to create quota manager");
...
async fn test_resource_quota_enforcement() {
    ...
    assert!(quota_manager
        .check_resource_quota(&resource_request1)
        .await
        .is_ok());

    // Update usage after first VM
    quota_manager
        .update_resource_usage(&tenant_id, 2, 2048, 10, 1)
        .await
    ...
    match quota_manager.check_resource_quota(&resource_request2).await {
        Err(QuotaViolation::CpuLimitExceeded {
            limit,
            current,
            requested,
        }) => {
    ...
    assert!(quota_manager
        .check_resource_quota(&resource_request3)
        .await
        .is_ok());
}
...
async fn test_per_node_vm_limits() {
    ...
    quota_manager
        .update_resource_usage(&tenant_id, 1, 1024, 5, 1)
        .await
        .expect("Failed to update usage");
    ...
    match quota_manager.check_resource_quota(&request3).await {
        Err(QuotaViolation::PerNodeVmLimitExceeded {
            node_id,
            limit,
            current,
        }) => {
...
async fn test_api_rate_limiting() {
    ...
    assert!(quota_manager
        .check_rate_limit(&tenant_id, &ApiOperation::VmCreate)
        .await
        .is_ok());
    quota_manager
        .record_api_request(&tenant_id, &ApiOperation::VmCreate)
        .await;
    ...
    match quota_manager
        ...
        .await
    {
        Err(QuotaViolation::RateLimitExceeded {
            operation,
            limit,
            current,
        }) => {
...
async fn test_concurrent_request_limiting() {
    ...
    assert!(quota_manager
        .check_rate_limit(&tenant_id, &ApiOperation::VmStart)
        .await
        .is_ok());
    quota_manager
        .record_api_request(&tenant_id, &ApiOperation::VmStart)
        .await;

    // Third request should be rejected (exceeds concurrent limit)
    match quota_manager
        .check_rate_limit(&tenant_id, &ApiOperation::StatusQuery)
        .await
    {
        Err(QuotaViolation::RateLimitExceeded {
            operation,
...
async fn test_quota_persistence() {
    ...
    {
        let storage = Arc::new(RedbRaftStorage {
...
async fn test_usage_tracking_accuracy() {
    ...
    quota_manager
        .update_resource_usage(&tenant_id, 4, 4096, 20, 2)
        .await
        .expect("Failed to update usage");
...
async fn test_quota_reset() {
    ...
    // Set up some usage
    quota_manager
        .update_resource_usage(&tenant_id, 4, 4096, 20, 1)
        .await
        .expect("Failed to update usage");
...
async fn test_multiple_tenants() {
    ...
    quota_manager
        .set_tenant_quota(quota1)
        .await
        .expect("Failed to set quota1");
    quota_manager
        .set_tenant_quota(quota2)
        .await
        .expect("Failed to set quota2");

    // Use resources for tenant1
    quota_manager
        .update_resource_usage(&tenant1, 2, 2048, 10, 1)
        .await
        .expect("Failed to update usage");

    // Use resources for tenant2
    quota_manager
        .update_resource_usage(&tenant2, 4, 4096, 20, 1)
        .await
        .expect("Failed to update usage");
