...
async fn test_three_node_cluster_formation_comprehensive() {
    ...
    for node_id in 1..=3 {
        let client = cluster.client(node_id).await
            .expect(&format!("Failed to get client for node {}", node_id));
        
        let (leader_id, nodes, _term) = client.get_cluster_status().await
            .expect(&format!("Failed to get cluster status from node {}", node_id));
        
        println!("Node {} sees leader: {}, nodes: {:?}", node_id, leader_id, nodes.len());
        
        // Verify this node sees all 3 nodes
        assert_eq!(nodes.len(), 3, "Node {} should see 3 nodes in cluster", node_id);
        
        // Verify all nodes have P2P info
        for node_info in &nodes {
            assert!(!node_info.p2p_node_id.is_empty(), 
                "Node {} should have P2P node ID", node_info.id);
            assert!(!node_info.p2p_addresses.is_empty() || !node_info.p2p_relay_url.is_empty(),
                "Node {} should have P2P addresses or relay URL", node_info.id);
        }
    }
    ...
    let create_response = leader_client.create_vm(vm_config).await
        .expect("Failed to create VM through leader");
    
    println!("Successfully created VM through leader: {:?}", create_response);
    ...
    let new_node_id = cluster.add_node().await
        .expect("Failed to add fourth node");
    ...
    let new_client = cluster.client(new_node_id).await
        .expect("Failed to get client for new node");
    
    let (_leader_id, nodes, _term) = new_client.get_cluster_status().await
    ...
    for node_info in &nodes {
        if node_info.id != new_node_id {
            assert!(!node_info.p2p_node_id.is_empty(), 
                "New node should have P2P info for node {}", node_info.id);
        }
    }
    ...
    let vms = new_client.list_vms().await
    ...
    cluster.remove_node(new_node_id).await
    ...
    for node_id in 1..=3 {
        ...
        let (_leader_id, nodes, _term) = client.get_cluster_status().await
            .expect(&format!("Failed to get cluster status from node {}", node_id));
        
        assert_eq!(nodes.len(), 3, "Node {} should see 3 nodes after removal", node_id);
    }
...
async fn test_cluster_formation_stress() {
    ...
    for i in 1..=3 {
        ...
        let (_leader_id, nodes, _term) = leader_client.get_cluster_status().await
