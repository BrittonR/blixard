...
async fn test_vm_scheduling_multi_node_placement() {
    ...
    for node in &summary.nodes {
        assert!(node.running_vms >= 1 && node.running_vms <= 4,
            "Node {} should have 1-4 VMs but has {}", node.node_id, node.running_vms);
    }
    ...
    let placement_gpu = leader.shared_state.schedule_vm_placement(
        &gpu_vm, 
        PlacementStrategy::Manual { node_id: 3 }
    ).await.expect("Should schedule to GPU node");
...
async fn test_vm_scheduling_with_node_failures() {
    ...
    for i in 1..=3 {
        leader.shared_state.register_worker_through_raft(i, format!("127.0.0.1:{}", 7000 + i), capabilities.clone()).await
...
async fn test_cluster_resource_summary_accuracy() {
    ...
    // Get initial summary
    let summary_initial = leader.shared_state.get_cluster_resource_summary().await
    ...
    let summary_after = leader.shared_state.get_cluster_resource_summary().await
    ...
    leader.shared_state.update_vm_status_through_raft("vm1".to_string(), VmStatus::Stopped, 1).await
        .expect("Should update VM status");
    
    timing::robust_sleep(Duration::from_millis(500)).await;
    
    let summary_stopped = leader.shared_state.get_cluster_resource_summary().await
...
async fn test_vm_migration_scheduling() {
    ...
    let db = leader.shared_state.get_database().await.expect("Should have database");
    let read_txn = db.begin_read().expect("Should begin read transaction");
    let vm_table = read_txn.open_table(blixard_core::storage::VM_STATE_TABLE).expect("Should open VM table");
    ...
    for vm_result in vm_table.iter().expect("Should iterate VMs") {
        ...
        let vm_state: blixard_core::types::VmState = bincode::deserialize(vm_data.value()).expect("Should deserialize VM state");
        vm_states.push(vm_state);
    }
    
    let vm_state = vm_states.iter().find(|v| v.name == "migratable-vm")
    ...
    let alternative_placement = leader.shared_state.schedule_vm_placement(&vm, PlacementStrategy::MostAvailable).await
        .expect("Should get alternative placement");
    
    // Should suggest node 2 or 3 (less loaded)
    assert_ne!(alternative_placement.selected_node_id, 1, "Should suggest different node");
...
async fn test_concurrent_vm_scheduling() {
    ...
    println!("Concurrent VM creation: {} successes, {} failures", successes, failures);
    ...
    for node in &summary.nodes {
        assert!(node.running_vms >= 2 && node.running_vms <= 6,
            "Node {} should have 2-6 VMs but has {}", node.node_id, node.running_vms);
    }
