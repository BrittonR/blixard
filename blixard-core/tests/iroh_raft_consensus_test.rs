...
async fn test_raft_consensus_over_iroh() -> BlixardResult<()> {
    ...
    assert!(leader_views.iter().all(|(_, leader)| *leader == first_leader),
            "Not all nodes agree on the leader");
    
    // Test state replication by creating a VM through Raft
    let leader_node = cluster.nodes().get(&leader_id)
    ...
    for (node_id, node) in cluster.nodes().iter() {
        ...
        assert!(tasks.iter().any(|t| t.id == task.id),
                "Node {} doesn't have the replicated task", node_id);
    ...
    println!("Initial peers: {:?}", initial_peers.iter().map(|p| p.id).collect::<Vec<_>>());
