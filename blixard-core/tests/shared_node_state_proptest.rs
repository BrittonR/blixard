...
fn arb_socket_addr() -> impl Strategy<Value = SocketAddr> {
    (any::<[u8; 4]>(), 1000u16..10000).prop_map(|(ip, port)| SocketAddr::from((ip, port)))
}
...
fn arb_node_config() -> impl Strategy<Value = NodeConfig> {
    (
        ...
        .prop_map(
            |(id, bind_addr, data_dir, join_addr, use_tailscale)| NodeConfig {
                id,
                bind_addr,
                data_dir,
                join_addr,
...
fn arb_raft_status() -> impl Strategy<Value = RaftStatus> {
    (
        ...
        .prop_map(|(is_leader, node_id, leader_id, term, state)| RaftStatus {
            is_leader,
            node_id,
            leader_id,
            term,
...
fn arb_concurrent_operation() -> impl Strategy<Value = ConcurrentOperation> {
    prop_oneof![
        ...
        (1u64..100, any::<bool>())
            .prop_map(|(id, connected)| ConcurrentOperation::UpdatePeerConnection(id, connected)),
