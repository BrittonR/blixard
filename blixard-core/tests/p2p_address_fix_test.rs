...
async fn test_p2p_address_collection_fixed() {
    ...
    assert!(
        ...
        "Peer should have P2P addresses - the address collection fix should ensure this"
    );
    ...
    for addr_str in &peer.p2p_addresses {
        assert!(
            ...
            "P2P address {} should be a valid socket address",
            addr_str
        );
    }
    ...
    println!(
        "Peer has {} P2P addresses: {:?}",
        peer.p2p_addresses.len(),
        peer.p2p_addresses
    );
}
