...
use blixard_core::{
    error::BlixardResult,
    iroh_types::{TaskSchedulingRequest, VmConfig as IrohVmConfig},
...
async fn test_p2p_cluster_formation_and_operations() -> BlixardResult<()> {
    ...
    let schedule_response = leader_client
        .clone()
        .schedule_vm_placement(schedule_request)
        .await?;
