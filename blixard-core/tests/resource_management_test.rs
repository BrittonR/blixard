...
use blixard_core::{
    resource_management::{
        ...
        ClusterResourceManager, PhysicalResources, AllocatedResources,
    },
    vm_scheduler::{VmScheduler, PlacementStrategy},
    types::VmConfig,
    raft_manager::WorkerCapabilities,
    storage::{WORKER_TABLE, WORKER_STATUS_TABLE},
};
use redb::Database;
use std::sync::Arc;
use tempfile::TempDir;

async fn create_test_scheduler_with_policy(policy: OvercommitPolicy) -> (VmScheduler, Arc<Database>, TempDir) {
    ...
    let _ = write_txn.open_table(blixard_core::storage::VM_STATE_TABLE).unwrap();
...
async fn test_conservative_overcommit_policy() {
    let (scheduler, database, _temp_dir) = create_test_scheduler_with_policy(
        OvercommitPolicy::conservative()
    ).await;
    ...
    // Actually allocate the resources (simulating VM creation)
    {
        let write_txn = database.begin_write().unwrap();
        {
            ...
            vm_table.insert(vm1.name.as_str(), bincode::serialize(&vm_state).unwrap().as_slice()).unwrap();
        }
        write_txn.commit().unwrap();
    }
...
async fn test_resource_reservations() {
    ...
    scheduler.create_system_reservation(
        1,  // node_id
        2,  // CPU cores
        4096,  // Memory MB
        20,  // Disk GB
...
async fn test_overcommit_capable_nodes() {
    ...
    scheduler.update_node_overcommit_policy(2, OvercommitPolicy::aggressive())
        .await
        .unwrap();
...
async fn test_resource_utilization_tracking() {
    ...
    let utilization = scheduler.get_node_resource_utilization(1)
        .await
        .unwrap()
        .unwrap();
