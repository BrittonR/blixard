...
async fn test_token_based_enrollment() -> BlixardResult<()> {
    ...
    let result2 = manager.enroll_with_token(
        node_id2,
        &token.token_id,
        &token.secret,
    ).await;
...
async fn test_multi_use_token_enrollment() -> BlixardResult<()> {
    ...
    let result4 = manager.enroll_with_token(
        node_id4,
        &token.token_id,
        &token.secret,
    ).await;
    ...
    assert!(result4.unwrap_err().to_string().contains("usage limit exceeded"));
...
async fn test_certificate_based_enrollment() -> BlixardResult<()> {
    ...
    let result1 = manager.enroll_with_certificate(node_id1, cert_attrs1).await?;
    ...
    let result2 = manager.enroll_with_certificate(node_id2, cert_attrs2).await?;
    ...
    assert!(result3.unwrap_err().to_string().contains("No matching certificate role mappings"));
...
async fn test_enrollment_state_persistence() -> BlixardResult<()> {
    ...
    // Create manager and generate tokens
    {
        ...
        manager.generate_enrollment_token(
            "user1".to_string(),
            vec!["role1".to_string()],
            "tenant1".to_string(),
            Duration::from_secs(3600),
    ...
    {
        let manager2 = IdentityEnrollmentManager::new(
            registry.clone(),
            None,
            Some(state_path),
        );
...
async fn test_concurrent_enrollment() -> BlixardResult<()> {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let node_id = NodeId::new(&[i as u8; 32]);
            manager_clone.enroll_with_token(node_id, &token_id, &secret).await
        });
    ...
    let token_info = tokens.iter().find(|t| t.token_id == token.token_id).unwrap();
