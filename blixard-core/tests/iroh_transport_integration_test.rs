...
async fn test_iroh_bidirectional_communication() -> BlixardResult<()> {
    ...
    // Get addresses
    let addr1 = NodeAddr::new(endpoint1.node_id())
        .with_direct_addresses(endpoint1.bound_sockets());
    let _addr2 = NodeAddr::new(endpoint2.node_id())
        .with_direct_addresses(endpoint2.bound_sockets());
            ...
            if let Ok(connecting) = incoming.accept() {
                if let Ok(conn) = connecting.await {
                    if let Ok((mut send, mut recv)) = conn.accept_bi().await {
                        ...
                        write_message(&mut send, MessageType::Response, header.request_id, &response).await?;
                    }
                }
            }
        }
    ...
    let conn = endpoint2.connect(addr1, b"blixard/1").await
        .map_err(|e| blixard_core::error::BlixardError::Internal {
...
async fn test_iroh_concurrent_connections() -> BlixardResult<()> {
    ...
    let server_addr = NodeAddr::new(server.node_id())
        .with_direct_addresses(server.bound_sockets());
                    ...
                    if let Ok(conn) = connecting.await {
                        if let Ok((mut send, mut recv)) = conn.accept_bi().await {
                            ...
                            if let Ok((_, payload)) = read_message(&mut recv).await {
                                ...
                                // Echo back
                                let _ = write_message(&mut send, MessageType::Response, generate_request_id(), &payload).await;
                            }
                        }
                    }
                }
    ...
    for i in 0..num_clients {
        ...
        let task = tokio::spawn(async move {
            ...
            let conn = client.connect(server_addr, b"blixard/1").await
                .map_err(|e| blixard_core::error::BlixardError::Internal {
            ...
            write_message(&mut send, MessageType::Request, generate_request_id(), &Bytes::from(msg.as_bytes().to_vec())).await?;
...
async fn test_iroh_large_message_transfer() -> BlixardResult<()> {
            ...
            if let Ok(connecting) = incoming.accept() {
                if let Ok(conn) = connecting.await {
                    if let Ok((mut send, mut recv)) = conn.accept_bi().await {
                        ...
                        write_message(&mut send, MessageType::Response, generate_request_id(), &ack).await?;
                    }
                }
            }
        }
    ...
    write_message(&mut send, MessageType::Request, generate_request_id(), &large_message).await?;
