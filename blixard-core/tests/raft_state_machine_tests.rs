...
use blixard_core::raft_manager::{
    ProposalData, RaftStateMachine, ResourceRequirements, TaskResult, TaskSpec, WorkerCapabilities,
    WorkerStatus,
};
use blixard_core::storage::{
    TASK_ASSIGNMENT_TABLE, TASK_RESULT_TABLE, TASK_TABLE, WORKER_STATUS_TABLE, WORKER_TABLE,
};
use blixard_core::types::{NodeTopology, VmCommand, VmConfig, VmStatus};
...
async fn test_update_nonexistent_worker_status() {
    ...
    let status_table = read_txn.open_table(WORKER_STATUS_TABLE).unwrap();
    let status_bytes = status_table
        .get(999u64.to_le_bytes().as_slice())
        .unwrap()
        .unwrap();
...
async fn test_multiple_vm_operations() {
    ...
    state_machine
        .apply_entry(&create_entry(create))
        .await
        .unwrap();
    ...
    state_machine
        .apply_entry(&create_entry(update))
        .await
        .unwrap();
    ...
    state_machine
        .apply_entry(&create_entry(delete))
        .await
        .unwrap();
