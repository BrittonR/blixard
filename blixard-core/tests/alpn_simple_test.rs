...
async fn test_alpn_constant_exists() {
    ...
    info!("BLIXARD_ALPN constant verified: {:?}", std::str::from_utf8(BLIXARD_ALPN).unwrap());
}
    ...
    match endpoint1.connect(node_addr2, BLIXARD_ALPN).await {
        Ok(conn) => {
            ...
            if let Ok(accepted) = timeout(Duration::from_secs(1), acceptor).await {
                assert!(accepted.unwrap(), "Acceptor should have accepted the connection");
            }
        }
        Err(e) => {
            // Connection might fail in test environment, but not due to ALPN mismatch
            info!("Connection failed (may be expected in test environment): {}", e);
            assert!(!e.to_string().contains("alpn"), "Error should not be ALPN-related: {}", e);
        }
    }
...
async fn test_alpn_mismatch_detection() {
    ...
    info!("Attempting connection with wrong ALPN: {:?}", std::str::from_utf8(wrong_alpn).unwrap());
