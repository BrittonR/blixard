...
use blixard_core::linearizability::{
    ...
    workload::{WorkloadGenerator, WorkloadConfig, OperationMix, patterns},
    failure_injection::{FailureInjector, FailureScenario},
    analysis::{HistoryAnalyzer, ViolationPattern, generate_html_report},
};
...
async fn test_vm_operations_linearizable() {
    ...
    for client_id in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            generator_clone.run_client(client_id, |op| {
    ...
    println!("Throughput: {:.1} ops/sec", report.performance.throughput_ops_per_sec);
    println!("Average latency: {:.1}ms", report.performance.avg_latency_ms);
...
async fn test_vm_operations_with_partition() {
    ...
    failure_injector.lock().await.load_scenario(partition_scenario);
        ...
        let handle = tokio::spawn(async move {
            generator_clone.run_client(client_id, |op| {
                ...
                async move {
                    ...
                    if !injector.lock().await.can_communicate(client_node, leader_node).await {
                    ...
                    // Add network delay
                    let delay = injector.lock().await.get_delay(client_node, leader_node).await;
    ...
    let has_split_brain = report.violations.iter().any(|v| {
        matches!(v, ViolationPattern::SplitBrain { .. })
    });
...
async fn test_byzantine_vm_operations() {
    ...
    failure_injector.lock().await.load_scenario(byzantine_scenario);
...
async fn test_analysis_and_reporting() {
    ...
    history.end_operation(id1, Response::VmCreated { success: true, node_id: Some(1) });
    ...
    history.end_operation(id2, Response::VmCreated { success: true, node_id: Some(2) });
