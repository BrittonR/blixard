...
use blixard_core::{
    ...
    storage::{WORKER_TABLE, WORKER_STATUS_TABLE, VM_STATE_TABLE},
};
use redb::Database;
use std::sync::Arc;
...
async fn test_hard_anti_affinity_constraint() {
    ...
    let web_rules = AntiAffinityRules::new()
        .add_rule(AntiAffinityRule::hard("web-app"));
...
async fn test_soft_anti_affinity_preference() {
    ...
    let cache_rules = AntiAffinityRules::new()
        .add_rule(AntiAffinityRule::soft("cache-layer", 1.0));
...
async fn test_anti_affinity_with_max_per_node() {
    ...
    let db_rules = AntiAffinityRules::new()
        .add_rule(AntiAffinityRule::hard("database").with_max_per_node(2));
...
async fn test_manual_placement_with_anti_affinity() {
    ...
    let api_rules = AntiAffinityRules::new()
        .add_rule(AntiAffinityRule::hard("api-server"));
