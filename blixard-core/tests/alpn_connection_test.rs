...
async fn test_alpn_basic_connection() {
    ...
    let receiver_task = tokio::spawn(async move {
        ...
        transport2_clone
            .accept_connections(move |doc_type, data| {
                info!(
                    "Node 2 received data: doc_type={:?}, size={}",
                    doc_type,
                    data.len()
                );
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            info!(
                "Direct connection failed (expected in test environment): {}",
                e
            );
        }
    }
...
async fn test_alpn_health_check() {
    ...
    let health_check_task = tokio::spawn(async move {
        ...
        transport2_clone
            .accept_health_check_connections()
            .await
            .unwrap();
    });
...
async fn test_alpn_endpoint_creation() {
    ...
    match endpoint.connect(self_addr, BLIXARD_ALPN).await {
        ...
        Err(e) => {
            ...
            info!(
                "Connection failed (might be expected for self-connection): {}",
                e
            );
            assert!(
                !e.to_string().contains("alpn"),
                "Should not have ALPN mismatch error"
            );
        }
    }
    ...
    for i in 0..3 {
        ...
        let handle = tokio::spawn(async move {
            ...
            match transport_clone
                .send_to_peer(&addr_clone, DocumentType::Metrics, &data)
                .await
            {
