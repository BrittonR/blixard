...
async fn test_iroh_transport_creation() {
    ...
    assert!(
        transport.is_ok(),
        "Should create IrohTransport successfully"
    );
    ...
    assert!(
        !addr.node_id.to_string().is_empty(),
...
async fn test_p2p_manager_creation() {
    ...
    assert!(
        manager.is_err(),
        "P2pManager creation should fail due to unimplemented document operations"
    );
...
async fn test_p2p_image_store_creation() {
    ...
    assert!(
        store.is_err(),
        "P2pImageStore creation should fail due to unimplemented document operations"
    );
    if let Err(e) = store {
...
async fn test_document_operations_return_not_implemented() {
    ...
    if let Err(e) = result {
        ...
        assert!(
            e.to_string().contains("not implemented") || e.to_string().contains("NotImplemented")
        );
    }

    let result = transport
        .write_to_doc(DocumentType::ClusterConfig, "test", b"data")
        .await;
    ...
    let result = transport
        .read_from_doc(DocumentType::ClusterConfig, "test")
        .await;
