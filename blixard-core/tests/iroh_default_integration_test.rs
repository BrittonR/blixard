...
use blixard_core::{
    ...
    transport::config::{TransportConfig, IrohConfig},
    error::BlixardResult,
};
...
async fn test_single_node_with_iroh_default() -> BlixardResult<()> {
    ...
    match &config.transport_config {
        ...
        _ => panic!("Expected Iroh transport but got {:?}", config.transport_config),
    }
...
async fn test_three_node_cluster_with_iroh() -> BlixardResult<()> {
    ...
    for i in 1..=3 {
        ...
        match &config.transport_config {
            ...
            _ => panic!("Node {} expected Iroh transport but got {:?}", i, config.transport_config),
        }
    }
    ...
    let (leader_id, members, _term) = cluster.get_node(1)
        .unwrap()
        .get_cluster_status()
        .await?;
...
async fn test_node_join_with_iroh() -> BlixardResult<()> {
    ...
    }).await.expect("Initial cluster should form");
    ...
    match &config.transport_config {
        ...
        _ => panic!("New node expected Iroh transport but got {:?}", config.transport_config),
    }
    
    // Wait for new node to join
    timeout(Duration::from_secs(10), async {
        loop {
            let (_leader, members, _) = cluster.get_node(1)
                .unwrap()
                .get_cluster_status()
                .await
                .unwrap();
    ...
    }).await.expect("Node should join cluster");
...
async fn test_explicit_iroh_config() -> BlixardResult<()> {
    ...
    test_node.node_mut().shared().config.transport_config = Some(TransportConfig::Iroh(iroh_config.clone()));
...
async fn test_raft_messages_over_iroh() -> BlixardResult<()> {
    ...
    }).await.expect("Cluster should form");
    ...
    for i in 0..5 {
        ...
        cluster.create_vm_on_node(leader_id, &vm_name, 1, 512).await?;
    }
