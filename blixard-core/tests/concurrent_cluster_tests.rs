...
async fn test_leader_change_during_join() {
    ...
    let initial_leader_id = {
        let leader_client = cluster.leader_client().await.expect("Should have a leader");
    ...
    println!(
        "Creating load on leader node {} to potentially trigger election...",
        initial_leader_id
    );
    ...
    let load_handle = tokio::spawn(async move {
        for i in 0..10 {
            ...
            if let Ok(shared_state) = cluster_for_load
                .get_node_shared_state(initial_leader_id)
                .await
            {
    ...
    match join_result {
        Ok(Ok(node_id)) => {
            println!(
                "✓ Node {} joined successfully despite potential leader change",
                node_id
            );
        }
        Ok(Err(e)) => {
    ...
    // Check final state
    let mut final_leader_client = cluster
        .leader_client()
        .await
    ...
    println!(
        "Final state - Leader: {}, Nodes: {} total",
        final_status.leader_id,
        final_status.nodes.len()
    );
    ...
    assert_eq!(
        final_status.nodes.len(),
        4,
        "Should have 4 nodes after join"
    );
            ...
            for j in 0..5 {
                ...
                let result = match cluster_clone.get_node_shared_state(node_id).await {
                    Ok(shared_state) => shared_state
                        .create_vm_through_raft(vm_command)
                        .await
                        .map(|_| ()),
                    Err(e) => Err(e),
                };
...
async fn test_concurrent_cluster_joins() {
    ...
    for handle in join_handles {
        match handle.await {
            Ok((task_id, result, duration)) => match result {
                Ok(node_id) => {
                    println!(
                        "✓ Task {} successfully added node {} in {:?}",
    ...
    println!(
        "\nConcurrent join results: {} successes, {} failures",
        successes, failures
    );
    ...
    assert!(
        node_ids.len() >= 2,
        "At least one concurrent join should succeed"
    );
    assert!(
        successes >= 1,
        "At least one concurrent join should succeed"
