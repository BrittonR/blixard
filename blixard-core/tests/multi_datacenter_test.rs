...
use blixard_core::types::{VmConfig, NodeTopology, LocalityPreference};
use blixard_core::raft_manager::WorkerCapabilities;
use blixard_core::storage::{WORKER_TABLE, WORKER_STATUS_TABLE, NODE_TOPOLOGY_TABLE, VM_STATE_TABLE};
use redb::Database;
use std::sync::Arc;
...
async fn test_datacenter_topology_storage() {
    ...
    // Store topologies
    {
        let write_txn = db.begin_write().unwrap();
        {
            ...
            topology_table.insert(1u64.to_le_bytes().as_slice(), data1.as_slice()).unwrap();
            
            let data2 = bincode::serialize(&topology2).unwrap();
            topology_table.insert(2u64.to_le_bytes().as_slice(), data2.as_slice()).unwrap();
        }
        write_txn.commit().unwrap();
    }
    ...
    // Read back topologies
    {
        ...
        let data1 = topology_table.get(1u64.to_le_bytes().as_slice()).unwrap().unwrap();
        ...
        let data2 = topology_table.get(2u64.to_le_bytes().as_slice()).unwrap().unwrap();
...
async fn test_locality_aware_placement() {
    ...
    // Register workers in different datacenters
    {
        let write_txn = db.begin_write().unwrap();
        {
            ...
            worker_table.insert(node1_id.to_le_bytes().as_slice(), worker_data1.as_slice()).unwrap();
            status_table.insert(node1_id.to_le_bytes().as_slice(), [0u8].as_slice()).unwrap();
            topology_table.insert(node1_id.to_le_bytes().as_slice(), bincode::serialize(&topology1).unwrap().as_slice()).unwrap();
            ...
            worker_table.insert(node2_id.to_le_bytes().as_slice(), worker_data2.as_slice()).unwrap();
            status_table.insert(node2_id.to_le_bytes().as_slice(), [0u8].as_slice()).unwrap();
            topology_table.insert(node2_id.to_le_bytes().as_slice(), bincode::serialize(&topology2).unwrap().as_slice()).unwrap();
            ...
            worker_table.insert(node3_id.to_le_bytes().as_slice(), worker_data3.as_slice()).unwrap();
            status_table.insert(node3_id.to_le_bytes().as_slice(), [0u8].as_slice()).unwrap();
            topology_table.insert(node3_id.to_le_bytes().as_slice(), bincode::serialize(&topology3).unwrap().as_slice()).unwrap();
        }
        write_txn.commit().unwrap();
    }
    ...
    let decision1 = scheduler.schedule_vm_placement(&vm_config1, PlacementStrategy::MostAvailable).await.unwrap();
    ...
    let decision2 = scheduler.schedule_vm_placement(&vm_config2, PlacementStrategy::MostAvailable).await.unwrap();
    ...
    let decision3 = scheduler.schedule_vm_placement(&vm_config3, PlacementStrategy::MostAvailable).await.unwrap();
...
async fn test_datacenter_latency_tracking() {
    ...
    scheduler.update_datacenter_latency("us-east-1", "us-west-1", 50).await;
    scheduler.update_datacenter_latency("us-east-1", "eu-west-1", 100).await;
    scheduler.update_datacenter_latency("us-west-1", "eu-west-1", 150).await;
    
    // Verify latencies are stored bidirectionally
    assert_eq!(scheduler.get_datacenter_latency("us-east-1", "us-west-1").await, Some(50));
    assert_eq!(scheduler.get_datacenter_latency("us-west-1", "us-east-1").await, Some(50));
    assert_eq!(scheduler.get_datacenter_latency("us-east-1", "eu-west-1").await, Some(100));
    assert_eq!(scheduler.get_datacenter_latency("eu-west-1", "us-east-1").await, Some(100));
    assert_eq!(scheduler.get_datacenter_latency("us-west-1", "eu-west-1").await, Some(150));
    assert_eq!(scheduler.get_datacenter_latency("eu-west-1", "us-west-1").await, Some(150));
    
    // Non-existent route
    assert_eq!(scheduler.get_datacenter_latency("us-east-1", "ap-southeast-1").await, None);
}