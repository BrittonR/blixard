...
impl RaftManager {
    ...
    pub fn new(
        ...
        let state_machine = Arc::new(RaftStateMachine::new(database.clone(), shared_state.clone()));
        ...
        let final_proposal_tx = Self::setup_batch_processing(
            node_id,
            &shared_state,
            proposal_tx.clone(),
        )?;
    ...
    async fn on_ready(&self) -> BlixardResult<bool> {
        ...
        self.snapshot_manager.process_snapshot_if_present(&ready, &self.storage).await?;
        ...
        self.snapshot_manager.check_and_trigger_log_compaction(&mut node, &self.storage).await?;
    ...
    /// Save entries to storage
    async fn save_entries_to_storage(&self, ready: &Ready, node: &mut RawNode<RedbRaftStorage>) -> BlixardResult<()> {
    ...
    async fn process_committed_entries(&self, ready: &mut Ready, node: &mut RawNode<RedbRaftStorage>) -> BlixardResult<u64> {
    ...
    async fn process_conf_change_entry(&self, entry: &Entry, node: &mut RawNode<RedbRaftStorage>) -> BlixardResult<()> {
        ...
        self.config_manager.apply_conf_change(&cc, entry, node).await?;
        
        Ok(())
    }

    /// Advance the Raft node and handle light ready
    async fn advance_raft_node(&self, node: &mut RawNode<RedbRaftStorage>, ready: Ready, last_applied_index: u64) -> BlixardResult<()> {
    ...
    async fn handle_newly_committed_entries(&self, node: &mut RawNode<RedbRaftStorage>, mut last_applied_index: u64, had_commit_update: bool) -> BlixardResult<()> {
    ...
    async fn send_raft_message(&self, msg: raft::prelude::Message) -> BlixardResult<()> {
        ...
        debug!("Sending Raft message: to={}, from={}, type={:?}", to, from, msg_type);
    ...
    pub async fn report_snapshot(&self, to: u64, status: raft::SnapshotStatus) -> BlixardResult<()> {
        let mut node = self.raft_node.write().await;
        self.snapshot_manager.report_snapshot(&mut node, to, status).await
    }
    ...
    pub async fn configure_admission_controller(&self, config: crate::resource_admission::AdmissionControlConfig) {
        ...
        warn!(self.logger, "configure_admission_controller called - needs interior mutability implementation");
        // TODO: Implement this with proper interior mutability
    }
}