...
impl RaftSnapshotManager {
    ...
    pub async fn process_snapshot_if_present(&self, ready: &Ready, storage: &RedbRaftStorage) -> BlixardResult<()> {
        if !ready.snapshot().is_empty() {
            info!(self.logger, "[RAFT-SNAPSHOT] Applying snapshot from ready state");
    ...
    pub fn apply_snapshot_to_state_machine(&self, snapshot_data: &[u8]) -> BlixardResult<()> {
        ...
        debug!("Applying snapshot with {} VM states, {} workers, {} tasks", 
        ...
        write_txn.commit().storage_context("commit snapshot transaction")?;
    ...
    pub async fn check_and_trigger_log_compaction(
        ...
        if applied_index > COMPACTION_THRESHOLD {
            ...
            if should_compact {
                ...
                self.perform_log_compaction(raft_node, storage, applied_index).await?;
            }
        }
    ...
    pub async fn report_snapshot(&self, raft_node: &mut RawNode<RedbRaftStorage>, to: u64, status: SnapshotStatus) -> BlixardResult<()> {
    ...
    fn clear_and_restore_string_table(
        ...
        debug!("Restored {} entries to {} table", snapshot_data.len(), table_name);
        Ok(())
    }
