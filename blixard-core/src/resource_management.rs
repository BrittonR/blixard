...
impl NodeResourceState {
    ...
    pub fn effective_capacity(&self) -> (u32, u64, u64) {
        ...
        let effective_cpu = ((physical.cpu_cores as f64) * policy.cpu_overcommit_ratio * reserve_factor) as u32;
        let effective_memory = ((physical.memory_mb as f64) * policy.memory_overcommit_ratio * reserve_factor) as u64;
        let effective_disk = ((physical.disk_gb as f64) * policy.disk_overcommit_ratio * reserve_factor) as u64;
    ...
    pub fn release(&mut self, cpu: u32, memory: u64, disk: u64) {
        ...
        self.allocated_resources.memory_mb = self.allocated_resources.memory_mb.saturating_sub(memory);
    ...
    pub fn add_reservation(&mut self, reservation: ResourceReservation) -> BlixardResult<()> {
        ...
        if reservation.is_hard && !self.can_allocate(
            ...
            return Err(BlixardError::InsufficientResources {
                requested: format!(
                    ...
                    reservation.id, reservation.cpu_cores, reservation.memory_mb, reservation.disk_gb
                ),
                available: {
                    let (a_cpu, a_mem, a_disk) = self.available_resources();
                    format!("CPU: {}, Memory: {}MB, Disk: {}GB", a_cpu, a_mem, a_disk)
    ...
    pub fn cleanup_expired_reservations(&mut self) {
        ...
        let expired_ids: Vec<String> = self.reservations
            .iter()
            .filter(|(_, res)| {
...
impl ClusterResourceManager {
    ...
    pub fn register_node(&mut self, node_id: u64, cpu_cores: u32, memory_mb: u64, disk_gb: u64) {
        let state = NodeResourceState::new(
            cpu_cores,
            memory_mb,
            disk_gb,
    ...
    pub fn update_node_policy(&mut self, node_id: u64, policy: OvercommitPolicy) -> BlixardResult<()> {
        self.node_states
            .get_mut(&node_id)
