...
impl Node {
    ...
    async fn initialize_internal(
        ...
        let security_manager =
            Arc::new(crate::security::SecurityManager::new(config.security.clone()).await?);
        ...
        if config.observability.metrics.enabled || config.observability.tracing.enabled {
            tracing::info!(
                "Initializing observability (metrics: {}, tracing: {})",
                config.observability.metrics.enabled,
                config.observability.tracing.enabled
            );
            let observability_manager = Arc::new(
                crate::observability::ObservabilityManager::new(config.observability.clone())
                    .await?,
            );
            self.shared
        ...
        if config.p2p.enabled || using_iroh_transport {
            tracing::info!(
                "Initializing P2P manager (P2P enabled: {}, Iroh transport: {})",
                config.p2p.enabled,
                using_iroh_transport
            );
        ...
        let (raft_manager, proposal_tx, message_tx, conf_change_tx, outgoing_rx) =
            RaftManager::new(node_id, db_arc.clone(), peers, Arc::downgrade(&self.shared))?;
        ...
        let p2p_monitor: Arc<dyn crate::p2p_monitor::P2pMonitor> =
            Arc::new(crate::p2p_monitor::NoOpMonitor);
        ...
        if let Ok(discovery_config) = Self::create_discovery_config(&self.shared.config) {
            ...
            if discovery_config.enable_static
                || discovery_config.enable_dns
                || discovery_config.enable_mdns
                || !discovery_config.static_nodes.is_empty()
            {
                tracing::info!(
                    "Initializing discovery manager (static={}, dns={}, mdns={}, static_nodes={})",
                    discovery_config.enable_static,
                    discovery_config.enable_dns,
                    discovery_config.enable_mdns,
                    discovery_config.static_nodes.len()
                );
                let mut discovery_manager =
                    crate::discovery::DiscoveryManager::new(discovery_config);
                        ...
                        if let Err(e) = transport_clone.send_message(to, msg).await {
                            ...
                            if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                                ...
                                if shared_weak.upgrade().is_none() {
                                    tracing::error!(
                                        "Node has been dropped, exiting outgoing message handler"
                                    );
                                    break;
                                }
        ...
        let raft_handle = tokio::spawn(async move {
            ...
            while restart_count < max_restarts {
                ...
                tokio::time::sleep(tokio::time::Duration::from_millis(
                    restart_delay_ms * restart_count as u64,
                ))
                .await;
                ...
                let peers: Vec<(u64, String)> =
                    peer_infos.into_iter().map(|p| (p.id, p.address)).collect();
                let node_id = shared.get_id();

                // Create new Raft manager
                match RaftManager::new(node_id, db.clone(), peers, Arc::downgrade(&shared)) {
                    Ok((
                        new_raft_manager,
                        proposal_tx,
                        message_tx,
                        ...
                        tokio::spawn(async move {
                            ...
                            loop {
                                match outgoing_rx.recv().await {
                                    Some((to, msg)) => {
                                        if let Some(transport) = &transport_for_handler {
                                            if let Err(e) = transport.send_message(to, msg).await {
                                                tracing::warn!(
                                                    "Failed to send Raft message to {}: {}",
                                                    to,
                                                    e
                                                );
                                                consecutive_errors += 1;
                                    ...
                                    None => {
                                        tracing::info!(
                                            "Outgoing channel closed in restarted handler"
                                        );
                                        break;
                                    }
                                }
        ...
        if let Some(join_addr) = self.shared.config.join_addr.clone() {
            ...
            if let Some(peer) = self.shared.get_peer(1).await {
                ...
                if let Some(peer_connector) = self.shared.get_peer_connector().await {
                    ...
                    tracing::info!(
                        "Pre-connected to leader at {} before sending join request",
                        join_addr
                    );
                }
            ...
            while start.elapsed() < timeout {
                if let Ok(status) = self.shared.get_raft_status().await {
                    if status.leader_id.is_some() {
                        tracing::info!(
                            "Leader identified: {:?}, proceeding with worker registration",
                            status.leader_id
                        );
                        leader_found = true;
                        break;
            ...
            if !leader_found {
                tracing::warn!(
                    "No leader identified after {:?}, attempting worker registration anyway",
                    timeout
                );
            }
