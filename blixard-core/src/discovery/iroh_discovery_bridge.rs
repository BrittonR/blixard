                        ...
                        match event {
                            ...
                            | DiscoveryEvent::NodeUpdated(info) => {
                                ...
                                if let Ok(node_addr) = Self::convert_to_node_addr(&info) {
                                    ...
                                    debug!(
                                        "Added/updated node {} in Iroh discovery cache",
                                        info.node_id
                                    );
                                }
                            }
            ...
            move |(cache, node_id, done)| async move {
                ...
                match nodes.get(&node_id) {
                    Some(node_addr) => {
                        ...
                        let direct_addresses: BTreeSet<_> =
                            node_addr.direct_addresses.iter().cloned().collect();
                        let mut node_info =
                            NodeInfo::new(node_id).with_direct_addresses(direct_addresses);
...
pub fn create_combined_discovery(discovery_manager: Arc<DiscoveryManager>) -> impl Discovery {
...
mod tests {
    ...
    fn test_node_addr_conversion() {
        ...
        assert_eq!(
            node_addr.relay_url.as_ref().unwrap().as_str(),
