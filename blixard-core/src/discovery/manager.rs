...
use crate::discovery::{
    dns_discovery::DnsDiscoveryProvider, mdns_discovery::MdnsDiscoveryProvider,
    static_discovery::StaticDiscoveryProvider, DiscoveryConfig, DiscoveryEvent, DiscoveryProvider,
    IrohNodeInfo,
};
...
impl DiscoveryManager {
    ...
    pub async fn start(&mut self) -> BlixardResult<()> {
        ...
        if self.config.enable_static {
            let static_nodes: Vec<(String, Vec<String>)> = self
                .config
                .static_nodes
                .iter()
            ...
            if !static_nodes.is_empty() {
                info!(
                    "Enabling static discovery with {} nodes",
                    static_nodes.len()
                );
        ...
        if self.config.enable_dns && !self.config.dns_domains.is_empty() {
            info!(
                "Enabling DNS discovery for domains: {:?}",
                self.config.dns_domains
            );
        ...
        if self.config.enable_mdns {
            info!(
                "Enabling mDNS discovery with service: {}",
        ...
        loop {
            ...
            for provider in providers.iter() {
                match provider.get_nodes().await {
                    ...
                    Err(e) => {
                        error!(
                            "Failed to get nodes from provider {}: {}",
                            provider.name(),
                            e
                        );
                    }
...
mod tests {
    ...
    async fn test_discovery_manager_with_static_nodes() {
        ...
        static_nodes.insert(node_id.to_string(), vec!["127.0.0.1:8080".to_string()]);
