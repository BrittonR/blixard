...
impl StaticDiscoveryProvider {
    ...
    async fn load_static_nodes(&self) -> BlixardResult<()> {
        for (node_id_str, addresses_str) in &self.static_nodes {
            ...
            info.metadata
                .insert("source".to_string(), "static".to_string());
            ...
            let _ = self
                .event_sender
                .send(DiscoveryEvent::NodeDiscovered(info))
                .await;

            debug!(
                "Added static node: {} with {} addresses",
                node_id_str,
                addresses_str.len()
            );
        }
...
mod tests {
    ...
    async fn test_static_discovery_basic() {
        ...
        let static_nodes = vec![(node_id_str.clone(), vec!["127.0.0.1:8080".to_string()])];
    ...
    async fn test_static_discovery_invalid_entries() {
        let static_nodes = vec![
            ...
            (
                NodeId::from_bytes(&[2u8; 32]).unwrap().to_string(),
    ...
    async fn test_static_discovery_events() {
        ...
        let static_nodes = vec![(node_id.to_string(), vec!["127.0.0.1:8080".to_string()])];
