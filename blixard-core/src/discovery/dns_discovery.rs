                        ...
                        if let Ok(node_id) = node_id_str.parse::<NodeId>() {
                            ...
                            match self.resolver.ipv4_lookup(srv.target().clone()).await {
                                Ok(ipv4_response) => {
                                    ...
                                    if !addresses.is_empty() {
                                        ...
                                        info.metadata.insert("source".to_string(), "dns".to_string());
                                        info.metadata.insert("domain".to_string(), domain.to_string());
                            ...
                            match self.resolver.ipv6_lookup(srv.target().clone()).await {
                                Ok(ipv6_response) => {
                                    ...
                                    if !addresses.is_empty() {
                                        if let Some(existing) = discovered_nodes.iter_mut()
    ...
    fn parse_txt_record(&self, text: &str, nodes: &mut Vec<IrohNodeInfo>) {
        ...
        if let Some(id) = node_id {
            if !addresses.is_empty() {
                ...
                info.metadata.insert("source".to_string(), "dns-txt".to_string());
                nodes.push(info);
            }
        }
    }
                ...
                match provider_read.query_domain(domain).await {
                    Ok(nodes) => {
                        for node in nodes {
                            ...
                            let _ = provider_read.event_sender.send(
...
impl DiscoveryProvider for DnsDiscoveryProvider {
    async fn start(&mut self) -> BlixardResult<()> {
        ...
        info!("Starting DNS discovery provider for domains: {:?}", self.domains);
            ...
            match self.query_domain(domain).await {
                Ok(nodes) => {
                    ...
                    for node in nodes {
                        ...
                        let _ = self.event_sender.send(DiscoveryEvent::NodeDiscovered(node)).await;
                    }
                }
                Err(e) => {
