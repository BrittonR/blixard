...
impl MdnsDiscoveryProvider {
    ...
    fn parse_service_info(&self, info: &ServiceInfo) -> Option<IrohNodeInfo> {
        ...
        for i in 0..10 {
            // Check up to 10 additional addresses
            if let Some(addr_str) = info
                .get_property(&format!("addr{}", i))
                .and_then(|val| Some(val.val_str()))
            {
        ...
        node_info
            .metadata
            .insert("source".to_string(), "mdns".to_string());

        Some(node_info)
    }

    /// Background task to handle mDNS events
    async fn mdns_task(provider: Arc<RwLock<Self>>, mut receiver: mpsc::Receiver<ServiceEvent>) {
                ...
                ServiceEvent::ServiceResolved(info) => {
                    ...
                    if let Some(node_info) = provider.parse_service_info(&info) {
                        ...
                        if Some(node_info.node_id) != provider.our_node_id {
                            ...
                            let _ = provider
                                .event_sender
                                .send(DiscoveryEvent::NodeDiscovered(node_info))
                                .await;
                        }
                    }
                }
...
impl DiscoveryProvider for MdnsDiscoveryProvider {
    async fn start(&mut self) -> BlixardResult<()> {
        ...
        info!(
            "Starting mDNS discovery provider with service: {}",
            self.service_name
        );
...
mod tests {
    ...
    async fn test_service_info_creation() {
        ...
        assert_eq!(
            info.get_properties().get("node_id").unwrap().to_string(),
            node_id.to_string()
        );
    }
