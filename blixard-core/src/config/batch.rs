...
impl BatchConfig {
    ...
    pub fn from_env() -> BlixardResult<Self> {
        ...
        config.batch_timeout =
            parse_duration_from_env("BLIXARD_BATCH_TIMEOUT_MS", config.batch_timeout);
    ...
    pub fn validate(&self) -> BlixardResult<()> {
        ...
        if self.max_batch_bytes > 100 * 1024 * 1024 {
            // 100MB
            return Err(BlixardError::ConfigError(
        ...
        if self.adaptive
            && (self.adaptive_min_size == 0 || self.adaptive_min_size > self.max_batch_size)
        {
            return Err(BlixardError::ConfigError(
    ...
    pub fn effective_batch_size(&self, current_latency: Duration) -> usize {
        ...
        if current_latency > self.adaptive_target_latency {
            ...
            std::cmp::max(self.adaptive_min_size, self.max_batch_size * 3 / 4)
