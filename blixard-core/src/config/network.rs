...
impl NetworkConfig {
    ...
    pub fn from_env() -> BlixardResult<Self> {
        ...
        config.request_timeout =
            parse_duration_secs_from_env("BLIXARD_REQUEST_TIMEOUT_SECS", config.request_timeout);
    ...
    pub fn validate(&self) -> BlixardResult<()> {
        ...
        if self.max_rpc_message_size < 1024 {
            // 1KB
            return Err(BlixardError::ConfigError(
        ...
        if self.max_rpc_message_size > 1024 * 1024 * 1024 {
            // 1GB
            return Err(BlixardError::ConfigError(
