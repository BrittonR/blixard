...
impl VmConfig {
    ...
    pub fn from_env() -> BlixardResult<Self> {
        ...
        config.startup_timeout = parse_duration_secs_from_env(
            "BLIXARD_VM_STARTUP_TIMEOUT_SECS",
        ...
        config.restart_delay = parse_duration_secs_from_env(
            "BLIXARD_VM_RESTART_DELAY_SECS",
    ...
    pub fn validate(&self) -> BlixardResult<()> {
        ...
        if !valid_strategies.contains(&self.scheduling.default_strategy.as_str()) {
            return Err(BlixardError::ConfigError(
                format!("Invalid scheduling strategy: {}", self.scheduling.default_strategy)
            ));
        }
