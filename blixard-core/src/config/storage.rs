...
impl StorageConfig {
    ...
    pub fn from_env() -> BlixardResult<Self> {
        ...
        if let Ok(val) = std::env::var("BLIXARD_CACHE_SIZE") {
            config.cache_size = val
                .parse()
                .map_err(|_| BlixardError::ConfigError("Invalid BLIXARD_CACHE_SIZE".to_string()))?;
        }
    ...
    pub fn validate(&self) -> BlixardResult<()> {
        ...
        if self.max_db_size < 100 * 1024 * 1024 {
            // 100MB minimum
            return Err(BlixardError::ConfigError(
        ...
        // Validate WAL buffer
        if self.wal_buffer_size < 1024 * 1024 {
            // 1MB minimum
            return Err(BlixardError::ConfigError(
        ...
        if self.backup.enabled {
            ...
            if self.backup.interval < Duration::from_secs(3600) {
                // 1 hour minimum
                return Err(BlixardError::ConfigError(
