...
impl NixVmImageServiceImpl {
    ...
    pub async fn import_microvm(
        ...
        let image_metadata = self
            .image_store
            .import_microvm(name, system_path, kernel_path)
            .await?;

        // Calculate deduplication ratio
        let total_chunks = image_metadata.chunk_hashes.len();
        let unique_chunks = image_metadata
            .chunk_hashes
            .iter()
    ...
    pub async fn import_container(
        ...
        let image_metadata = self
            .image_store
            .import_container(image_ref, tar_path)
            .await?;
    ...
    pub async fn import_closure(
        ...
        // Import the closure
        let image_metadata = self.image_store.import_closure(name, root_path).await?;
    ...
    pub async fn download_image(
        ...
        let (path, stats) = self
            .image_store
            .download_image(image_id, target_dir)
            .await?;
    ...
    pub async fn prefetch_for_migration(
        ...
        self.image_store
            .prefetch_for_migration(vm_name, target_node)
            .await?;
    ...
    pub async fn garbage_collect(
        ...
        self.image_store
            .garbage_collect(keep_recent, keep_min_copies)
            .await
    }
...
impl crate::transport::iroh_service::IrohService for NixVmImageServiceImpl {
    ...
    async fn handle_call(
        ...
        method: &str,
        payload: bytes::Bytes,
    ) -> BlixardResult<bytes::Bytes> {
        use crate::transport::iroh_protocol::{deserialize_payload, serialize_payload};
        ...
        match method {
            ...
            "download_image" => {
                ...
                let req: Request = deserialize_payload(&payload)?;
                let (path, stats) = self
                    .download_image(&req.image_id, req.target_dir.as_ref().map(|d| Path::new(d)))
                    .await?;
            ...
            "prefetch_migration" => {
                ...
                let req: Request = deserialize_payload(&payload)?;
                self.prefetch_for_migration(&req.vm_name, req.target_node)
                    .await?;
            ...
            "garbage_collect" => {
                ...
                let bytes_freed = self
                    .garbage_collect(req.keep_recent_days, req.keep_min_copies)
                    .await?;
...
impl<'a> NixVmImageClient<'a> {
    ...
    pub async fn download_image(
        ...
        Ok((
            resp.local_path,
            resp.bytes_transferred,
            resp.bytes_deduplicated,
            resp.duration_ms,
        ))
    }
}
