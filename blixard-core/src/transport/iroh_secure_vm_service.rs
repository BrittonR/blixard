...
impl SecureIrohVmService {
    ...
    async fn handle_authorized_request(
        ...
        match &request {
            ...
            VmRequest::Image(img) => {
                self.authorize_vm_image_operation(&auth_context, img)
                    .await?;
            }
        }
    ...
    async fn authorize_vm_operation(
        ...
        if !authorized {
            return Err(BlixardError::Security {
                message: format!(
                    "User {} not authorized for VM operation",
    ...
    async fn authorize_vm_image_operation(
        ...
        let authorized = match operation {
            ...
            VmImageRequest::GetImage { .. } | VmImageRequest::ListImages => {
        ...
        if !authorized {
            return Err(BlixardError::Security {
                message: format!(
                    "User {} not authorized for VM image operation",
                    auth_context.user_id
                ),
            });
        }
...
impl IrohService for SecureIrohVmService {
    ...
    async fn handle_call(&self, method: &str, payload: Bytes) -> BlixardResult<Bytes> {
        ...
        match method {
            ...
            _ => Err(BlixardError::P2PError(format!(
                "Unknown method: {}",
                method
            ))),
        }
