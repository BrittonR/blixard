...
{
    fn network_context(self, operation: &str) -> BlixardResult<T> {
        self.map_err(|e| BlixardError::NetworkError(format!("Network {}: {}", operation, e.into())))
    }
    ...
    fn p2p_context(self, operation: &str, peer_id: Option<&str>) -> BlixardResult<T> {
        ...
        self.map_err(|e| BlixardError::NetworkError(format!("{}: {}", context, e.into())))
    }

    fn connection_context(self, operation: &str, address: &str) -> BlixardResult<T> {
        self.map_err(|e| {
            BlixardError::NetworkError(format!(
                "Connection {} to {}: {}",
                operation,
                address,
                e.into()
    ...
    fn iroh_context(self, operation: &str) -> BlixardResult<T> {
        self.map_err(|e| BlixardError::NetworkError(format!("Iroh {}: {}", operation, e.into())))
    }
    ...
    E: Into<Box<dyn std::error::Error + Send + Sync>>,
{
    ...
    fn vm_scheduling_context(self, operation: &str, strategy: Option<&str>) -> BlixardResult<T> {
        self.map_err(|e| {
            ...
            BlixardError::SchedulingError { message: details }
        })
    }
}
...
mod tests {
    ...
    fn test_connection_context() {
        let result: Result<(), Box<dyn std::error::Error + Send + Sync>> = Err("timeout".into());
