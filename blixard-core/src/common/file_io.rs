...
pub async fn read_text_file_with_context<P: AsRef<Path>>(
    ...
    tokio::fs::read_to_string(path)
        ...
        .map_err(|e| match e {
            BlixardError::IoError(io_err) => BlixardError::ConfigError(
                format!("Failed to read {}: {} (path: {:?})", context, io_err, path)
            ),
            other => other,
        })
}
...
{
    let path = path.as_ref();
    let content = read_text_file_with_context(path, &format!("{} configuration", file_type)).await?;
    ...
    match extension {
        "json" => serde_json::from_str(&content).map_err(|e| BlixardError::ConfigError(
            format!("Failed to parse JSON {}: {}", file_type, e)
        )),
        "toml" => toml::from_str(&content).map_err(|e| BlixardError::ConfigError(
            format!("Failed to parse TOML {}: {}", file_type, e)
        )),
        "yaml" | "yml" => serde_yaml::from_str(&content).map_err(|e| BlixardError::ConfigError(
            format!("Failed to parse YAML {}: {}", file_type, e)
        )),
        _ => {
            ...
            if let Ok(parsed) = serde_json::from_str(&content) {
                ...
                Err(BlixardError::ConfigError(
                    format!("Unable to parse {} - unknown format", file_type)
                ))
            }
        }
    }
...
pub async fn write_text_file_with_context<P: AsRef<Path>>(
    ...
    if let Some(parent) = path.parent() {
        tokio::fs::create_dir_all(parent)
            ...
            .map_err(|e| BlixardError::ConfigError(
                format!("Failed to create directory for {}: {}", context, e)
            ))?;
    }
    ...
    tokio::fs::write(path, content)
        .await
        .map_err(|e| BlixardError::ConfigError(
            format!("Failed to write {}: {} (path: {:?})", context, e, path)
        ))
}
    ...
    let content = match extension {
        "json" => {
            if pretty {
                ...
                format!("Failed to serialize JSON {}: {}", file_type, e)
            ))?
        }
        "toml" => {
            if pretty {
                ...
                format!("Failed to serialize TOML {}: {}", file_type, e)
            ))?
        }
        "yaml" | "yml" => {
            serde_yaml::to_string(data).map_err(|e| BlixardError::ConfigError(
                format!("Failed to serialize YAML {}: {}", file_type, e)
            ))?
        }
        _ => {
...
pub async fn read_directory_with_context<P: AsRef<Path>>(
    ...
    let mut entries = tokio::fs::read_dir(path)
        .await
        .map_err(|e| BlixardError::ConfigError(
            format!("Failed to read {} directory {:?}: {}", context, path, e)
        ))?;
    ...
    while let Some(entry) = entries.next_entry()
        .await
        .map_err(|e| BlixardError::ConfigError(
            format!("Failed to read directory entry in {}: {}", context, e)
        ))?
    {
...
pub async fn read_files_with_extension<P: AsRef<Path>>(
    ...
    for path in paths {
        if path.extension().and_then(|s| s.to_str()) == Some(extension) {
            let content = read_text_file_with_context(&path, 
                &format!("{} file", extension)).await?;
...
mod tests {
    ...
    async fn test_config_file_json() {
        ...
        // Read
        let loaded: TestConfig = read_config_file(&file_path, "json")
            .await
            .unwrap();
    ...
    async fn test_config_file_toml() {
        ...
        // Read
        let loaded: TestConfig = read_config_file(&file_path, "toml")
            .await
            .unwrap();
    ...
    async fn test_error_handling() {
        let result = read_text_file_with_context("/non/existent/file", "test")
            .await;
