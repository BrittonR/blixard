...
use crate::{
    ...
    vm_health_types::{
        HealthCheck, HealthCheckResult, HealthState, VmHealthCheckConfig, VmHealthStatus,
    },
};
...
impl VmHealthMonitor {
    ...
    pub fn start(&mut self) {
        ...
        let handle = tokio::spawn(async move {
            info!(
                "Starting VM health monitor with interval: {:?}",
                check_interval
            );
            ...
            loop {
                ...
                if let Err(e) =
                    Self::run_health_checks_v2(&node_state, &vm_manager, &auto_recovery).await
                {
    ...
    async fn run_health_checks(
        ...
        for (vm_config, stored_status) in vms {
            ...
            if actual_status != stored_status {
                ...
                if let Err(e) = node_state
                    .update_vm_status_through_raft(vm_config.name.clone(), actual_status, node_id)
                    .await
                {
                ...
                if actual_status == VmStatus::Failed && stored_status == VmStatus::Running {
                    info!(
                        "Triggering auto-recovery for failed VM '{}'",
                        vm_config.name
                    );
                    if let Err(e) = auto_recovery
        ...
        if status_changes > 0 {
            metrics
                .vm_status_changes
                .add(status_changes, &[attributes::node_id(node_id)]);
        }
...
mod tests {
    ...
    async fn test_health_monitor_lifecycle() {
        ...
        let mut monitor = VmHealthMonitor::new(node_state, vm_manager, Duration::from_secs(5));
