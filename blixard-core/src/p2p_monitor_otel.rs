...
impl P2pMonitor for OtelP2pMonitor {
    async fn record_connection_attempt(&self, peer_id: &str, success: bool) {
        ...
        if !success {
            self.metrics.grpc_requests_failed.add(
                1,
                &[
    ...
    async fn record_connection_state_change(
        ...
        match (from, to) {
            (_, ConnectionState::Connected) => {
                self.connections_active
                    .add(1, &[KeyValue::new("state", "connected")]);
        ...
        match (from, to) {
            (_, ConnectionState::Connecting) => {
                self.connections_active
                    .add(1, &[KeyValue::new("state", "connecting")]);
            }
    ...
    async fn record_rtt(&self, peer_id: &str, rtt_ms: f64) {
        let attributes = vec![KeyValue::new("peer_id", peer_id.to_string())];
