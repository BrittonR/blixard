...
impl SharedNodeState {
    ...
    pub async fn set_vm_manager(&self, vm_manager: Arc<VmManager>) {
        let shared = Arc::new(VmManagerShared { inner: vm_manager });
    ...
    pub async fn set_security_manager(
        &self,
    ...
    pub async fn set_observability_manager(
        &self,
        observability_manager: Arc<crate::observability::ObservabilityManager>,
    ) {
    ...
    pub async fn get_observability_manager(
        &self,
    ) -> Option<Arc<crate::observability::ObservabilityManager>> {
    ...
    pub async fn set_peer_connector(
        &self,
    ...
    pub async fn get_peer_connector(
        &self,
    ...
    pub async fn set_vm_health_monitor(
        &self,
        monitor: Arc<crate::vm_health_monitor::VmHealthMonitor>,
    ) {
        *self.vm_health_monitor.write().await = Some(monitor);
    }

    /// Get the VM health monitor
    pub async fn get_vm_health_monitor(
        &self,
    ) -> Option<Arc<crate::vm_health_monitor::VmHealthMonitor>> {
        self.vm_health_monitor.read().await.clone()
    }

    /// Set the VM auto-recovery service
    pub async fn set_vm_auto_recovery(
        &self,
        recovery: Arc<crate::vm_auto_recovery::VmAutoRecovery>,
    ) {
        *self.vm_auto_recovery.write().await = Some(recovery);
    }

    /// Get the VM auto-recovery service
    pub async fn get_vm_auto_recovery(
        &self,
    ) -> Option<Arc<crate::vm_auto_recovery::VmAutoRecovery>> {
    ...
    pub async fn create_vm_through_raft(&self, command: VmCommand) -> BlixardResult<()> {
        ...
        if peers.is_empty() && !raft_status.is_leader {
            tracing::warn!(
                "Node has no peers and is not leader - rejecting write to prevent split-brain"
            );
            return Err(BlixardError::ClusterError(
        ...
        if !raft_status.is_leader {
            ...
            if !peers.is_empty() {
                ...
                tracing::info!(
                    "Leadership safety check passed: {}/{} nodes reachable",
                    reachable_count,
                    total_configured_nodes
                );
            }
            ...
            if let Some(tx) = proposal_tx.as_ref() {
                ...
                match tx.send(proposal) {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "Failed to send VM proposal: channel closed or full. Error: {:?}",
                            e
                        );
                        return Err(BlixardError::Internal {
                            message: "Failed to send VM proposal - Raft channel closed".to_string(),
    ...
    pub async fn send_vm_operation_through_raft(&self, command: VmCommand) -> BlixardResult<()> {
        ...
        if !raft_status.is_leader {
            ...
            match tokio::time::timeout(tokio::time::Duration::from_secs(timeout), response_rx).await
            {
                ...
                Ok(Err(_)) => Err(BlixardError::Internal {
                    message: "VM operation response channel closed".to_string(),
                }),
                Err(_) => Err(BlixardError::Internal {
    ...
    pub async fn get_task_status(
        &self,
        task_id: &str,
    ) -> BlixardResult<Option<(String, Option<TaskResult>)>> {
        let database = self.database.read().await;
    ...
    pub async fn add_peer_with_p2p(
        &self,
        id: u64,
        address: String,
    ...
    pub async fn update_peer_p2p_info(
        &self,
        id: u64,
        p2p_node_id: Option<String>,
        p2p_addresses: Vec<String>,
    ...
    pub async fn propose_conf_change(
        ...
        self.propose_conf_change_with_p2p(change_type, node_id, address, None, Vec::new(), None)
            .await
    }
    ...
    pub async fn propose_conf_change_with_p2p(
        ...
        tracing::info!(
            "[NODE-SHARED] Proposing conf change: {:?} for node {} at {}",
            change_type,
            node_id,
            address
        );
        ...
        if !is_leader {
            ...
            tracing::error!(
                "[NODE-SHARED] Cannot propose conf change - not the leader. Current leader: {:?}",
                leader_id
            );
            return Err(BlixardError::ClusterError(format!(
                "Only the leader can propose configuration changes. Current leader: {:?}",
                leader_id
            )));
        }
        ...
        if let Some(sender) = tx.as_ref() {
            ...
            match sender.send(conf_change) {
                Ok(_) => {
                    tracing::info!(
                        "[NODE-SHARED] Conf change sent successfully, waiting for response"
                    );
                }
                Err(e) => {
            ...
            let timeout = crate::config_global::get()
                ...
                .as_secs();
            tracing::info!(
                "[NODE-SHARED] Waiting for conf change response from Raft (timeout: {}s)",
                timeout
            );
    ...
    pub async fn register_worker_through_raft(
        ...
        if !raft_status.is_leader {
            tracing::warn!(
                "[NODE-SHARED] Worker registration attempted on non-leader node {}, leader is {:?}",
                raft_status.node_id,
            ...
            return Err(BlixardError::ClusterError(format!(
                "Only the leader can register workers. Current leader: {:?}",
                raft_status.leader_id
            )));
        }
        ...
        if let Some(sender) = tx.as_ref() {
            match sender.send(proposal) {
                ...
                Err(e) => {
                    tracing::error!(
                        "[NODE-SHARED] Failed to send worker registration proposal: {}",
                        e
                    );
                    return Err(BlixardError::Internal {
    ...
    pub async fn update_vm_status_through_raft(
        ...
        tracing::info!(
            "update_vm_status_through_raft called for VM '{}' to status {:?}",
            vm_name,
            status
        );
        ...
        if !raft_status.is_leader {
            ...
            if let Some(tx) = proposal_tx.as_ref() {
                ...
                match tx.send(proposal) {
                    Ok(_) => {
                        tracing::info!(
                            "Successfully sent VM status update proposal to Raft channel"
                        );
                    }
                    Err(e) => {
                        ...
                        return Err(BlixardError::Internal {
                            message:
                                "Failed to send VM status update proposal - Raft channel closed"
                                    .to_string(),
                        });
                    }
                }
                    ...
                    .await
                {
                    Ok(Ok(result)) => {
                        tracing::info!(
                            "Received VM status update proposal response: {:?}",
                            result.is_ok()
                        );
                        result?;
                        Ok(())
                    ...
                    Ok(Err(_)) => {
                        ...
                        Err(BlixardError::Internal {
                            message: "VM status update proposal response channel closed"
                                .to_string(),
                        })
                    }
                    Err(_) => {
                        tracing::error!(
                            "VM status update proposal timed out after {} seconds",
                            timeout
                        );
                        Err(BlixardError::Internal {
