...
pub async fn replace_legacy_quota_system() -> BlixardResult<()> {
    ...
    composite
        .add_manager(ResourceType::Memory, Box::new(memory_manager))
        .await?;
    composite
        .add_manager(ResourceType::Cpu, Box::new(cpu_manager))
        .await?;
    ...
    info!(
        "✓ Allocated resources: {} memory, {} CPU",
        memory_allocation.allocation_id, cpu_allocation.allocation_id
    );
    ...
    composite
        .deallocate(&memory_allocation.allocation_id)
        .await?;
...
pub async fn migrate_from_legacy_system() -> BlixardResult<()> {
    ...
    migration_wrapper
        .migrate_from_quota_manager(&legacy_quotas)
        .await?;
    ...
    let can_allocate = migration_wrapper
        .legacy_quota_check("tenant-1", 2048, 4)
        .await?;
    info!("Legacy quota check result: {}", can_allocate);

    // Step 7: Allocate resources using unified interface
    if can_allocate {
        let allocations = migration_wrapper
            .allocate_for_vm("legacy-vm-1", "tenant-1", 2048, 4)
            .await?;
...
pub async fn advanced_resource_patterns() -> BlixardResult<()> {
    ...
    info!(
        "Memory efficiency: {:.1}% allocation, {:.1}% utilization",
    ...
    let reservation_request = ResourceAllocationRequest {
        ...
        metadata: [("job_type".to_string(), "batch_processing".to_string())]
            .iter()
            .cloned()
            .collect(),
    };
    ...
    info!(
        "✓ Created temporary reservation: {}",
        reservation.allocation_id
    );

    // Activate the reservation when needed
    memory_manager
        .activate_reservation(&reservation.allocation_id)
        .await?;
    ...
    // Clean up
    memory_manager
        .deallocate(&reservation.allocation_id)
        .await?;
    ...
    for (vm_name, memory_mb, vcpus) in vm_requirements.iter() {
        ...
        if memory_available && cpu_available {
            ...
            let memory_alloc_request = ResourceAllocationRequest {
                ...
                is_temporary: false,
                expires_at: None,
                metadata: [("vm_name".to_string(), vm_name.to_string())]
                    .iter()
                    .cloned()
                    .collect(),
            };
...
mod tests {
    ...
    async fn test_replace_legacy_quota_system() {
        ...
        assert!(
            result.is_ok(),
            "Legacy quota system replacement failed: {:?}",
            result
        );
    }
    ...
    async fn test_migrate_from_legacy_system() {
        ...
        assert!(
            result.is_ok(),
            "Legacy system migration failed: {:?}",
            result
        );
    }
    ...
    async fn test_advanced_resource_patterns() {
        ...
        assert!(
            result.is_ok(),
            "Advanced resource patterns failed: {:?}",
            result
        );
    }
    ...
    async fn test_vm_scheduling_integration() {
        ...
        assert!(
            result.is_ok(),
            "VM scheduling integration failed: {:?}",
            result
        );
    }
}
