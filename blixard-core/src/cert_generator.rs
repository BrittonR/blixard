...
use rcgen::{
    ...
    KeyPair, SanType, BasicConstraints, IsCa, KeyUsagePurpose,
...
impl CertGenerator {
    ...
    pub async fn generate_cluster_pki(
        ...
        for node_name in node_names {
            ...
            let bundle = self.generate_server_cert(&node_name, san_list, &ca_cert).await?;
    ...
    async fn generate_ca(&self, common_name: &str) -> BlixardResult<(Certificate, CertificateBundle)> {
        ...
        params.not_after = params.not_before + Duration::days(self.config.validity_days).to_std()
        ...
        // Save to files
        self.save_certificate(&bundle, &format!("ca-{}", common_name)).await?;
        
        info!("Generated CA certificate with serial: {}", bundle.serial_number);
        Ok((cert, bundle))
    }
    ...
    async fn generate_server_cert(
        ...
        params.subject_alt_names = san_list.into_iter()
            .map(|san| {
        ...
        // Save to files
        self.save_certificate(&bundle, &format!("server-{}", common_name)).await?;
        
        info!("Generated server certificate with serial: {}", bundle.serial_number);
        Ok(bundle)
    }
    ...
    async fn generate_client_cert(
        ...
        // Save to files
        self.save_certificate(&bundle, &format!("client-{}", common_name)).await?;
        
        info!("Generated client certificate with serial: {}", bundle.serial_number);
        Ok(bundle)
    }
    ...
    async fn save_certificate(&self, bundle: &CertificateBundle, name: &str) -> BlixardResult<()> {
        ...
        // Save certificate
        fs::write(&cert_path, &bundle.cert_pem).await
            .map_err(|e| BlixardError::Security {
                message: format!("Failed to write certificate file: {}", e),
            })?;
        
        // Save private key with restricted permissions
        fs::write(&key_path, &bundle.key_pem).await
            .map_err(|e| BlixardError::Security {
...
pub async fn generate_cluster_pki(
    ...
    let _pki = generator.generate_cluster_pki(cluster_name, node_names, client_names).await?;
...
mod tests {
    ...
    async fn test_server_cert_generation() {
        ...
        let server_bundle = generator.generate_server_cert(
            "node1.blixard.local",
            vec![
                "node1.blixard.local".to_string(),
                "127.0.0.1".to_string(),
            ],
            &ca_cert,
        ).await.unwrap();
    ...
    async fn test_client_cert_generation() {
        ...
        let client_bundle = generator.generate_client_cert(
            "admin@blixard.local",
            &ca_cert,
        ).await.unwrap();
