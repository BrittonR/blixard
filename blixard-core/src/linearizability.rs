...
use crate::{
    ...
    iroh_types::{Response, VmConfig, VmInfo, VmState, CreateVmRequest, CreateVmResponse, 
                 StartVmRequest, StartVmResponse, StopVmRequest, StopVmResponse,
                 ListVmsResponse, GetVmStatusRequest, GetVmStatusResponse},
};
...
pub enum OperationResult {
    ...
    VmStatus { exists: bool, running: bool },
    ClusterStatus { leader: Option<u64>, nodes: Vec<u64> },
}
...
impl RecordingClient {
    pub fn new(client: IrohClusterServiceClient, history: OperationHistory, client_id: u64) -> Self {
        Self {
            inner: client,
            history,
            client_id,
    ...
    pub async fn create_vm(&self, config: VmConfig) -> BlixardResult<Response<CreateVmResponse>> {
        let op_id = self.history.begin(
            OperationType::CreateVm { name: config.name.clone() },
            self.client_id
        ).await;
        ...
        let op_result = match &result {
            Ok(resp) => if resp.get_ref().success {
                ...
                OperationResult::Failure { error: resp.get_ref().message.clone() }
            },
            Err(e) => OperationResult::Failure { error: e.to_string() },
        };
    ...
    pub async fn stop_vm(&self, name: String) -> BlixardResult<Response<StopVmResponse>> {
        let op_id = self.history.begin(
            OperationType::StopVm { name: name.clone() },
            self.client_id
        ).await;
    ...
    fn check_vm_consistency(&self, entries: &[HistoryEntry]) -> Result<(), String> {
        ...
        for entry in entries {
            match (&entry.operation, &entry.result) {
                (
                    OperationType::CreateVm { name },
                    Some(OperationResult::Success)
                ) => {
                    if !created_vms.insert(name.clone()) {
                        return Err(format!(
                            "VM '{}' created successfully multiple times", name
                        ));
                    }
                },
                _ => {},
...
mod tests {
    ...
    async fn test_operation_history_recording() {
        ...
        let id = history.begin(
            OperationType::CreateVm { name: "test-vm".to_string() },
            1
        ).await;
        ...
        assert!(matches!(completed[0].result, Some(OperationResult::Success)));
    }
}