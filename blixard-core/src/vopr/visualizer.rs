...
impl Visualizer {
    ...
    fn write_operation<W: Write>(&self, writer: &mut W, op: &Operation) -> std::io::Result<()> {
        match op {
            ...
            Operation::ClockDrift {
                ...
                write!(
                    writer,
                    "ClockDrift(node={}, rate={}ppm)",
                    node_id, drift_rate_ppm
                )?;
            }
            Operation::ByzantineNode { node_id, behavior } => {
                write!(
                    writer,
                    "ByzantineNode(node={}, behavior={:?})",
                    node_id, behavior
                )?;
            }
            _ => {
    ...
    fn write_state_progression<W: Write>(
        ...
        writeln!(
            writer,
            "Time  | Nodes | Leader | MaxView | MaxCommit | Partitions | Messages"
        )?;
        writeln!(
            writer,
            "------|-------|--------|---------|-----------|------------|----------"
        ...
        for (i, snapshot) in snapshots.iter().enumerate() {
            ...
            // Node count
            let running_nodes = snapshot.nodes.values().filter(|n| n.is_running).count();
    ...
    fn write_events<W: Write>(&self, writer: &mut W, events: &[Event]) -> std::io::Result<()> {
        ...
        for (node_id, node_state) in &state.nodes {
            ...
            if node_state.is_running {
                ...
                write!(
                    writer,
                    " | View: {}",
                    state.views.get(node_id).unwrap_or(&0)
                )?;
                write!(
                    writer,
                    " | Commit: {}",
                    state.commit_points.get(node_id).unwrap_or(&0)
                )?;
    ...
    pub fn generate_reproducer<W: Write>(
        ...
        writeln!(
            writer,
            "// Minimal reproducer for bug found with seed {}",
            seed
        )?;
...
mod tests {
    ...
    fn test_basic_visualization() {
        ...
        visualizer
            .generate_report(&mut output, 42, &operations, &[], &[], None)
            .unwrap();
