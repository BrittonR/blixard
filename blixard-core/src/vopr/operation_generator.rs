...
impl OperationGenerator {
    ...
    pub fn generate_weighted(&mut self, weights: &OperationWeights, rng: &mut ChaCha8Rng) -> Operation {
        let total_weight = weights.start_node + weights.stop_node + weights.restart_node
            + weights.client_request + weights.network_partition + weights.network_heal
            + weights.clock_jump + weights.clock_drift + weights.message_drop
            + weights.message_duplicate + weights.message_reorder + weights.byzantine_behavior;
        ...
        check_weight!(weights.network_partition, self.generate_network_partition(rng));
        ...
        check_weight!(weights.message_duplicate, self.generate_message_duplicate(rng));
        check_weight!(weights.message_reorder, self.generate_message_reorder(rng));
        check_weight!(weights.byzantine_behavior, self.generate_byzantine_behavior(rng));
...
mod tests {
    ...
    fn test_operation_generation() {
        ...
        let has_client_ops = ops.iter().any(|op| matches!(op, Operation::ClientRequest { .. }));
