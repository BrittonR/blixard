...
pub use self::time_accelerator::{TimeAccelerator, SimulatedTime};
pub use self::fuzzer_engine::{FuzzerEngine, FuzzConfig, FuzzMode};
pub use self::invariants::{Invariant, SafetyInvariant, LivenessInvariant, InvariantChecker};
pub use self::state_tracker::{StateTracker, StateSnapshot};
pub use self::operation_generator::{OperationGenerator, Operation};
pub use self::test_harness::{TestHarness, ClusterState};
...
impl Vopr {
    ...
    pub fn generate_reproducer(&self, failure: &FuzzFailure) -> String {
        ...
        let ops = failure.minimal_reproducer.as_ref()
            .unwrap_or(&failure.operations);
        
        visualizer.generate_reproducer(
            &mut output,
            failure.seed,
            ops,
        ).unwrap();
