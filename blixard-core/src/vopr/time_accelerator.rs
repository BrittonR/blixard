...
impl NodeClock {
    ...
    fn now(&self, global_time: SimulatedTime) -> SimulatedTime {
        ...
        let elapsed = global_time.duration_since(&self.last_update)
            .unwrap_or(Duration::ZERO);
        let drift_nanos = (elapsed.as_secs() as i64) * self.drift_rate;
        
        // Apply skew and drift
        let adjusted_nanos = (global_time.nanos as i128) + (self.skew as i128) + (drift_nanos as i128);
...
impl TimeAccelerator {
    ...
    pub fn clock_skew_stats(&self) -> ClockSkewStats {
        ...
        let times: Vec<i128> = clocks.values()
            .map(|clock| {
