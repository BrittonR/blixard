...
impl LatencyTracker {
    ...
    pub fn complete_operation(&mut self, request_id: u64) -> Option<Duration> {
        if let Some((op_type, start)) = self.in_flight.remove(&request_id) {
            ...
            self.latencies.entry(op_type).or_insert_with(Vec::new).push(duration);
            Some(duration)
        } else {
            None
        }
        ...
        if let Some(prev) = self.snapshots.back() {
            ...
            for anomaly in anomalies {
                self.record_event(Event {
                    ...
                    node_id: None,
                    event_type: EventType::AnomalyDetected { description: anomaly },
                    details: String::new(),
                });
            }
        }
    ...
    pub fn get_interesting_moments(&self) -> Vec<InterestingMoment> {
        ...
        for (i, snapshot) in self.snapshots.iter().enumerate() {
            if i > 0 {
                ...
                let prev_leaders: Vec<_> = prev.nodes.values()
                    .filter(|n| n.role == NodeRole::Leader)
                    .map(|n| n.id)
                    .collect();
                    
                let curr_leaders: Vec<_> = snapshot.nodes.values()
                    .filter(|n| n.role == NodeRole::Leader)
                    .map(|n| n.id)
                    .collect();
                ...
                if prev_leaders != curr_leaders {
                    moments.push(InterestingMoment {
                        ...
                        description: format!("Leader change: {:?} -> {:?}", prev_leaders, curr_leaders),
                        snapshot_index: i,
                    });
                }
                ...
                if snapshot.partitions.len() != prev.partitions.len() {
                    moments.push(InterestingMoment {
                        timestamp: snapshot.timestamp,
                        description: format!("Network partition change: {} -> {} partitions", 
                            prev.partitions.len(), snapshot.partitions.len()),
                        snapshot_index: i,
                    });
                }
            }
    ...
    fn check(&self, prev: &StateSnapshot, curr: &StateSnapshot) -> Vec<String> {
        ...
        for (node_id, node) in &curr.nodes {
            if let Some(prev_node) = prev.nodes.get(node_id) {
                ...
                if time_diff > self.heartbeat_interval.as_nanos() * 10 {
                    anomalies.push(format!("Node {} missing heartbeats for {}ms", 
                        node_id, time_diff / 1_000_000));
                }
            }
        }
        ...
        for (node_id, node) in &curr.nodes {
            if node.clock_skew_ms.abs() > self.max_clock_skew.as_millis() as i64 {
                anomalies.push(format!("Node {} has extreme clock skew: {}ms", 
                    node_id, node.clock_skew_ms));
            }
        }
        
        // Check for split brain (multiple leaders)
        let leaders: Vec<_> = curr.nodes.values()
            .filter(|n| n.role == NodeRole::Leader)
            .map(|n| n.id)
            .collect();
            
        if leaders.len() > 1 {
            anomalies.push(format!("Split brain detected: multiple leaders {:?}", leaders));
        }
        ...
        if max_commit == prev_max_commit && curr.messages_in_flight == 0 {
            ...
            anomalies.push("System appears stalled: no progress and no messages in flight".to_string());
        }
