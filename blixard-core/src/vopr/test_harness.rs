...
use crate::vopr::operation_generator::{Operation, ClientOp, ByzantineBehavior};
...
impl TestHarness {
    ...
    pub async fn execute_operation(&self, operation: &Operation) -> BlixardResult<()> {
        match operation {
            ...
            Operation::ClientRequest { client_id, request_id, operation } => {
                self.execute_client_operation(*client_id, *request_id, operation).await
            }
            Operation::NetworkPartition { partition_a, partition_b } => {
                self.create_network_partition(partition_a.clone(), partition_b.clone()).await
            }
            Operation::NetworkHeal => {
                self.heal_network().await
            }
            ...
            Operation::DropMessage { from, to, probability } => {
            ...
            Operation::ReorderMessages { from, to, window_size } => {
                self.configure_message_reorder(*from, *to, *window_size).await
            }
            Operation::CheckInvariant { invariant } => {
                // Invariants are checked after each operation in the main loop
                Ok(())
            }
            Operation::WaitForProgress { timeout_ms } => {
                self.wait_for_progress(*timeout_ms).await
            }
            _ => Ok(()),
        }
    }
    ...
    async fn start_node(&self, node_id: u64) -> BlixardResult<()> {
        ...
        cov.basic_blocks.insert(hash_operation("StartNode", node_id));
    ...
    async fn configure_message_drop(&self, from: u64, to: u64, probability: f64) -> BlixardResult<()> {
    ...
    async fn configure_message_duplicate(&self, from: u64, to: u64, count: u32) -> BlixardResult<()> {
    ...
    async fn configure_message_reorder(&self, from: u64, to: u64, window_size: u32) -> BlixardResult<()> {
        ...
        if let Some(cluster) = cluster_guard.as_ref() {
            ...
            for node_id in cluster.get_node_ids() {
                ...
                nodes.insert(node_id, NodeState {
                    ...
                    byzantine: self.byzantine_nodes.read().await.get(&node_id)
                        .map(|b| format!("{:?}", b)),
                });
