...
impl QuotaManager {
    ...
    pub async fn check_resource_quota(&self, request: &ResourceRequest) -> QuotaResult<()> {
        ...
        if let Some(node_id) = request.node_id {
            let current_vms_on_node = current_usage
                .vm_usage
                .vms_per_node
                .get(&node_id)
                .unwrap_or(&0);
    ...
    pub async fn check_rate_limit(
        ...
        let tenant_tracking = rate_limits
            .tenant_requests
        ...
        let operation_times = tenant_tracking
            .operation_tracking
        ...
        let operation_limit = match operation {
            ...
            ApiOperation::ClusterJoin => {
                quota.api_limits.operation_limits.cluster_join_per_hour / 60
            } // Per minute
            ApiOperation::StatusQuery => {
                quota.api_limits.operation_limits.status_query_per_second * 60
            } // Per minute
            ApiOperation::ConfigChange => {
                quota.api_limits.operation_limits.config_change_per_hour / 60
            } // Per minute
    ...
    pub async fn record_api_request_end(&self, tenant_id: &str) {
        ...
        if let Some(tenant_tracking) = rate_limits.tenant_requests.get_mut(tenant_id) {
            tenant_tracking.concurrent_requests =
                tenant_tracking.concurrent_requests.saturating_sub(1);
        }
    }
    ...
    pub async fn update_resource_usage(
        ...
        debug!(
            "Updated usage for tenant {}: vcpus={}, memory_mb={}, disk_gb={}, node_id={}",
    ...
    pub async fn reset_tenant_usage(&self, tenant_id: &str) -> BlixardResult<()> {
        ...
        usage.insert(
            tenant_id.to_string(),
            TenantUsage::new(tenant_id.to_string()),
        );
