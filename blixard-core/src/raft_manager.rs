...
use chrono::Utc;
use raft::prelude::*;
use raft::{Config, GetEntriesContext, RawNode, StateRole};
...
impl RaftStateMachine {
    ...
    pub async fn apply_entry(&self, entry: &Entry) -> BlixardResult<()> {
        ...
        let proposal: ProposalData =
            bincode::deserialize(&entry.data).map_err(|e| BlixardError::Serialization {
            ...
            ProposalData::Batch(proposals) => {
                ...
                for sub_proposal in proposals {
                    ...
                    let sub_entry = Entry {
                        ...
                        term: entry.term,
                        index: entry.index,
                        data: bincode::serialize(&sub_proposal).map_err(|e| {
    ...
    fn apply_assign_task(
...

        {
            ...
            let task_data = bincode::serialize(task).map_err(|e| BlixardError::Serialization {
    ...
    fn apply_complete_task(
...

        {
            ...
            let result_data =
                bincode::serialize(result).map_err(|e| BlixardError::Serialization {
...

        {
            ...
            match command {
                VmCommand::Create { config, node_id } => {
                    ...
                    let needs_ip = vm_config.ip_address.is_some();
                    ...
                    let vm_data =
                        bincode::serialize(&vm_state).map_err(|e| BlixardError::Serialization {
                ...
                VmCommand::UpdateStatus { name, status } => {
                    let serialized = {
                        ...
                        if let Some(data) = vm_state_data {
                            let mut vm_state: crate::types::VmState =
                            ...
                            Some(bincode::serialize(&vm_state).map_err(|e| {
                                BlixardError::Serialization {
                                    operation: "serialize vm state".to_string(),
                                    source: Box::new(e),
                                }
                    ...
                    let has_networking = {
                        ...
                        if let Some(data) = vm_data {
                            if let Ok(vm_state) = bincode::deserialize::<crate::types::VmState>(data.value()) {
                                vm_state.config.ip_address.is_some()
                            } else {
                                false
                            }
                        } else {
                            ...
                            match &command {
                                VmCommand::Start { name } | VmCommand::Stop { name } => {
                                    if let Ok(Some(data)) = table.get(name.as_str()) {
                                        if let Ok(vm_state) =
                                            bincode::deserialize::<crate::types::VmState>(
                                                data.value(),
                                            )
                                        {
    ...
    async fn apply_ip_pool_command(
        ...
        match command {
            ...
            crate::ip_pool::IpPoolCommand::SetPoolEnabled { pool_id, enabled } => {
                ...
                let mut table = txn.open_table(IP_POOL_TABLE)?;
                let updated_data = if let Some(data) = table.get(&pool_id.0)? {
                    ...
                    Some(bincode::serialize(&config).map_err(|e| {
                        BlixardError::Serialization {
                            operation: "serialize ip pool config".to_string(),
                            source: Box::new(e),
                        }
                ...
                if let Some(pool_data) = updated_data {
...
impl RaftManager {
    pub fn new(
        ...
        let final_proposal_tx = if let Some(_shared) = shared_state.upgrade() {
            let config = config_global::get()
                .map_err(|e| BlixardError::Internal { message: format!("Failed to get config: {}", e) })?;
        ...
        loop {
            ...
            // This can happen in single-node clusters where commits happen in light ready
            {
                ...
                if node.raft.raft_log.committed > node.raft.raft_log.applied {
                    ...
                    info!(
                        self.logger,
                        "[RAFT-LOOP] Triggering tick to process unapplied commits"
                    );
                    self.tick().await?;
                    ...
                    if ready_count == 0 {
                        warn!(
                            self.logger,
                            "[RAFT-LOOP] No ready states after tick for unapplied commits"
                        );
                    } else {
                        info!(
                            self.logger,
                            "[RAFT-LOOP] Processed {} ready states after tick", ready_count
                        );
                    }
                }
            }
            ...
            if needs_trigger {
                ...
                if node.raft.state == StateRole::Leader {
                    info!(
                        self.logger,
                        "[RAFT-CONF] Triggering replication by proposing empty entry"
                    );
                    node.propose(vec![], vec![])?;
    ...
    async fn handle_proposal(&self, proposal: RaftProposal) -> BlixardResult<()> {
        ...
        metrics
            ...
            .add(1, &[attributes::node_id(self.node_id)]);
        ...
        match node.propose(proposal_id.clone(), data) {
            Ok(_) => {
                info!(
                    self.logger,
                    "[RAFT-PROPOSAL] Proposal submitted to Raft successfully"
                );
            }
            Err(e) => {
    ...
    async fn handle_raft_message(
        ...
        if let Some(shared) = self.shared_state.upgrade() {
            ...
            if !peers.contains_key(&from) {
                if let Some(peer_info) = shared.get_peer(from).await {
                    ...
                    info!(
                        self.logger,
                        "[RAFT-MSG] Added sender {} at {} to peer list from incoming {:?} message",
                        from,
                        peer_info.address,
                        msg.msg_type()
                    );
                    ...
                    if let Some(peer_connector) = shared.get_peer_connector().await {
                        self.connect_to_peer_with_p2p(peer_connector, &peer_info)
                            .await;
                    }
                }
            }
        ...
        node.step(msg).map_err(|e| BlixardError::Raft {
        ...
        if !msgs.is_empty() {
            info!(
                self.logger,
                "[RAFT-MSG] Found {} messages after step",
                msgs.len()
            );
            for msg in msgs {
    ...
    async fn handle_conf_change(&self, conf_change: RaftConfChange) -> BlixardResult<()> {
        ...
        let context = if matches!(conf_change.change_type, ConfChangeType::AddNode) {
            ...
            bincode::serialize(&ctx).map_err(|e| BlixardError::Serialization {
        ...
        if let Some(response_tx) = conf_change.response_tx {
            ...
            info!(
                self.logger,
                "[RAFT-CONF] Response channel stored successfully"
            );
        }
        ...
        if is_leader && is_single_node {
            info!(
                self.logger,
                "[RAFT-CONF] Single-node cluster, checking if conf change can be applied"
            );
            ...
            loop {
                ...
                if retries > 10 {
                    warn!(
                        self.logger,
                        "[RAFT-CONF] Conf change not committed after 10 retries"
                    );
                    break;
                }
            ...
            info!(
                self.logger,
                "[RAFT-CONF] Saved new configuration state to storage"
            );
            ...
            if matches!(conf_change.change_type, ConfChangeType::AddNode) {
                if let Ok(address) = bincode::deserialize::<String>(&cc.context) {
                    ...
                    if let Some(shared) = self.shared_state.upgrade() {
                        ...
                        if let Some(peer_connector) = shared.get_peer_connector().await {
                            if let Some(peer_info) = shared.get_peer(conf_change.node_id).await {
                                tracing::info!(
                                    "[RAFT-CONF] Proactively connecting to new peer {} at {}",
                                    peer_info.id,
                                    peer_info.address
                                );
                                self.connect_to_peer_with_p2p(peer_connector, &peer_info)
                    ...
                    *self.needs_replication_trigger.write().await = true;
                    info!(
                        self.logger,
                        "[RAFT-CONF] Set replication trigger flag for new node"
                    );
                    ...
                    info!(
                        self.logger,
                        "[RAFT-CONF] Waited for peer connection establishment"
                    );
                }
            }
            ...
            if let Some(response_tx) = pending.remove(&cc_id) {
                ...
                info!(
                    self.logger,
                    "[RAFT-CONF] Sent success response for conf change"
                );
            }
            ...
            if matches!(conf_change.change_type, ConfChangeType::AddNode) {
                *self.needs_replication_trigger.write().await = true;
                info!(
                    self.logger,
                    "[RAFT-CONF] Set flag to trigger replication to new node"
                );
            }
        }
    ...
    async fn process_snapshot_if_present(&self, ready: &raft::Ready) -> BlixardResult<()> {
    ...
    async fn save_entries_to_storage(&self, ready: &raft::Ready, node: &mut raft::RawNode<RedbRaftStorage>) -> BlixardResult<()> {
        if !ready.entries().is_empty() {
            ...
            if self.peers.read().await.is_empty() && node.raft.state == raft::StateRole::Leader {
    ...
    async fn send_messages_to_peers(&self, ready: &mut raft::Ready) -> BlixardResult<()> {
    ...
    async fn process_committed_entries(&self, ready: &mut raft::Ready, node: &mut raft::RawNode<RedbRaftStorage>) -> BlixardResult<u64> {
    ...
    async fn process_normal_entry(&self, entry: &raft::prelude::Entry) -> BlixardResult<()> {
    ...
    async fn process_conf_change_entry(&self, entry: &raft::prelude::Entry, node: &mut raft::RawNode<RedbRaftStorage>) -> BlixardResult<()> {
        ...
        self.apply_conf_change(&cc, entry, node).await?;
        
        Ok(())
    }

    /// Apply a configuration change (simplified version)
    /// Note: This is a simplified version of the original complex configuration change logic.
    /// The original version had extensive peer management, state reconstruction, and error handling.
    async fn apply_conf_change(&self, cc: &raft::prelude::ConfChange, _entry: &raft::prelude::Entry, node: &mut raft::RawNode<RedbRaftStorage>) -> BlixardResult<()> {
        info!(self.logger, "[RAFT-CONF] Applying ConfChange";
            "node_id" => cc.node_id,
            "change_type" => ?cc.change_type(),
            "context_len" => cc.context.len()
        );

        // Apply the configuration change to Raft
        let cs = node.apply_conf_change(cc)?;
        
        // Store the new configuration state
        self.storage.save_conf_state(&cs)?;
        
        info!(self.logger, "[RAFT-CONF] Configuration change applied"; 
            "voters" => ?cs.voters,
    ...
    /// Update the applied index after processing entries
    async fn update_applied_index(&self, node: &mut raft::RawNode<RedbRaftStorage>, last_applied_index: u64) -> BlixardResult<()> {
    ...
    async fn check_and_trigger_log_compaction(&self, node: &mut raft::RawNode<RedbRaftStorage>) -> BlixardResult<()> {
        const COMPACTION_THRESHOLD: u64 = 1000;
        
        let applied_index = node.raft.raft_log.applied;
        if applied_index > COMPACTION_THRESHOLD {
            ...
            if should_compact {
                ...
                // Create a snapshot
                match node.mut_store().snapshot(applied_index, 0) {
                    Ok(snapshot) => {
    ...
    fn log_ready_state_details(&self, ready: &raft::Ready, node: &raft::RawNode<RedbRaftStorage>) {
    ...
    async fn advance_raft_node(&self, node: &mut raft::RawNode<RedbRaftStorage>, ready: raft::Ready, last_applied_index: u64) -> BlixardResult<()> {
        ...
        self.handle_newly_committed_entries(node, last_applied_index, had_commit_update).await?;
        
        Ok(())
    }
    
    /// Handle newly committed entries that weren't in the original ready state
    async fn handle_newly_committed_entries(&self, node: &mut raft::RawNode<RedbRaftStorage>, mut last_applied_index: u64, had_commit_update: bool) -> BlixardResult<()> {
    ...
    async fn update_shared_state_status(&self, node: &raft::RawNode<RedbRaftStorage>) {
    ...
    async fn on_ready(&self) -> BlixardResult<bool> {
        ...
        self.process_snapshot_if_present(&ready).await?;
        ...
        let last_applied_index = self.process_committed_entries(&mut ready, &mut node).await?;

        // Advance the Raft node
        self.advance_raft_node(&mut node, ready, last_applied_index).await?;

        // Update applied index
        self.update_applied_index(&mut node, last_applied_index).await?;

        // Check and trigger log compaction if needed
        self.check_and_trigger_log_compaction(&mut node).await?;
    ...
    async fn send_raft_message(&self, msg: raft::prelude::Message) -> BlixardResult<()> {
        ...
        self.outgoing_messages.send((to, msg)).map_err(|_| {
    ...
    async fn connect_to_peer_with_p2p(
        &self,
        peer_connector: Arc<crate::transport::iroh_peer_connector::IrohPeerConnector>,
        peer_info: &crate::node_shared::PeerInfo,
    ) {
                ...
                if node_id_bytes.len() == 32 {
                    ...
                    if let Ok(node_id) = iroh::NodeId::from_bytes(&bytes) {
                        ...
                        tokio::spawn(async move {
                            ...
                            {
                                tracing::warn!(
                                    "[RAFT-MSG] Failed to connect to peer {} at {}: {}",
                                    peer_id,
                                    peer_address,
                                    e
                                );
    ...
    async fn check_and_send_snapshots(
        &self,
        ...
        let followers_needing_snapshot: Vec<u64> = node
            .raft
            .prs()
            .iter()
            .filter_map(|(id, pr)| {
    ...
    pub async fn propose(&self, data: ProposalData) -> BlixardResult<()> {
        ...
        // Wait for response
        rx.await.map_err(|_| BlixardError::Internal {
...
pub async fn schedule_task(
    ...
    for entry in worker_table.iter()? {
        ...
        let node_id = u64::from_le_bytes(node_id_bytes.value().try_into().map_err(|_| {
        ...
        let (_address, capabilities): (String, WorkerCapabilities) =
            bincode::deserialize(worker_data.value()).map_err(|e| BlixardError::Serialization {
                operation: "deserialize worker capabilities".to_string(),
                source: Box::new(e),
            })?;
        ...
        if capabilities.cpu_cores >= task.resources.cpu_cores
            ...
            && task
                ...
                .required_features
                .iter()
                .all(|f| capabilities.features.contains(f))
        {
    ...
    tracing::info!(
        "Total workers found: {}, best worker: {:?}",
        worker_count,
        best_worker
    );
