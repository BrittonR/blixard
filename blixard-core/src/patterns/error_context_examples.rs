...
use crate::patterns::error_context::{DomainErrorContext, ErrorCollector, ErrorContext};
use std::fs::File;
use std::io::Read;
use std::path::Path;
...
pub fn read_config_file(path: &Path) -> BlixardResult<String> {
    ...
    // After: With context
    let mut file =
        File::open(path).with_context(|| format!("Failed to open config file at {:?}", path))?;
...
pub async fn connect_to_peer(peer_id: &str, address: &str) -> BlixardResult<String> {
    ...
    let handshake_result = perform_handshake(peer_id)
        .await
...
pub async fn storage_operation(
    ...
    match operation {
        StorageOp::Get => get_from_storage(key)
            .await
        ...
        StorageOp::Delete => delete_from_storage(key)
            .await
...
pub async fn replicate_data_to_peers(
    ...
    for peer_id in peer_ids {
        let result = send_data_to_peer(peer_id, &data)
            .await
...
pub async fn access_secure_resource(
    ...
    // Check permissions
    check_permissions(user_id, resource_id, action)
        .await
    ...
    log_access_attempt(user_id, resource_id, action)
        .await
        .with_security_context("audit log", resource_id)?;

    // Access the resource
    fetch_secure_resource(resource_id)
        .await
...
pub fn process_batch_fast(items: &[Item]) -> BlixardResult<Vec<ProcessedItem>> {
    ...
    for item in items {
        ...
        let processed = process_item_internal(item).context("Failed to process batch item")?;
...
pub async fn handle_external_api_call(endpoint: &str) -> BlixardResult<String> {
    match call_external_api(endpoint).await {
        ...
        Err(e) => {
            ...
            match e.kind() {
                ExternalErrorKind::Timeout => Err(BlixardError::Timeout {
...
mod tests {
    ...
    fn test_config_validation_context() {
        ...
        assert!(error
            .to_string()
            .contains("Cluster must have at least one node"));
    }
}
