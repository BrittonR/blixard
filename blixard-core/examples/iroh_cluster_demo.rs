...
use blixard_core::transport::services::health::{HealthServiceImpl, HealthService};
use blixard_core::transport::services::status::{StatusServiceImpl, StatusService};
...
async fn create_cluster_nodes(
    ...
    for i in 1..=count {
        ...
        // Register services
        let health_service = IrohHealthService::new(
            HealthServiceImpl::new(node.clone())
        );
        server.register_service("health", Arc::new(health_service));
        
        let status_service = IrohStatusService::new(
            StatusServiceImpl::new(node.clone())
        );
    ...
    for (i, (node, _, _)) in nodes.iter().enumerate() {
        for (peer_id, peer_addr) in &node_addrs {
            if *peer_id != (i + 1) as u64 {
                node.add_peer(*peer_id, format!("127.0.0.1:{}", 7000 + peer_id)).await;
...
async fn test_health_checks(
    ...
    for i in 1..nodes.len() {
        ...
        match client.connect_to_service::<dyn HealthService>(target_addr).await {
            Ok(mut health_client) => {
                match health_client.check().await {
                    Ok(response) => {
                        let latency = start.elapsed();
                        println!("  Node 1 → Node {}: Healthy={}, Latency={:?}", 
                            i + 1, response.healthy, latency);
                    }
...
async fn test_status_queries(
    ...
    for (i, (_, endpoint, _)) in nodes.iter().enumerate() {
        ...
        match client.connect_to_service::<dyn StatusService>(target_addr).await {
            Ok(mut status_client) => {
                match status_client.get_cluster_status().await {
                    Ok(status) => {
                        println!("  Node {} sees {} cluster members", 
                            i + 1, status.nodes.len());
                    }
...
async fn test_message_performance(
    ...
    // Warm up connection
    if let Ok(mut health_client) = client.connect_to_service::<dyn HealthService>(target_addr.clone()).await {
