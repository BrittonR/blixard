...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n--- Test 2: Send blob info to Node 2 ---");
    transport1
        .send_blob_info(&addr2, hash, "shared.txt")
        .await?;
                ...
                if doc_type == DocumentType::FileTransfer {
                    if let Ok(message) = String::from_utf8(data.clone()) {
                        ...
                        if message.starts_with("BLOB:") {
                            // Parse blob info
                            let parts: Vec<&str> =
                                message.strip_prefix("BLOB:").unwrap().split(':').collect();
                            if parts.len() == 2 {
                                let hash_str = parts[0];
                                let filename = parts[1];
                                println!(
                                    "Received blob info - Hash: {}, Filename: {}",
                                    hash_str, filename
                                );
                                let _ =
                                    tx_clone.try_send((hash_str.to_string(), filename.to_string()));
                            }
                        }
                    }
                }
    ...
    match transport2
        .download_blob_from_peer(&addr1, hash, &output_file)
        .await
    {
        Ok(_) => {
