...
async fn main() -> Result<()> {
    ...
    transport3.join_doc_from_ticket(&ticket, DocumentType::ClusterConfig).await?;
    
    info!("All nodes have joined the cluster config document");
    
    // Subscribe to events on all nodes
    transport1.subscribe_to_doc_events(DocumentType::ClusterConfig).await?;
    transport2.subscribe_to_doc_events(DocumentType::ClusterConfig).await?;
    transport3.subscribe_to_doc_events(DocumentType::ClusterConfig).await?;
    
    // Give time for initial sync
    sleep(Duration::from_secs(1)).await;
    
    // Write initial cluster configuration
    transport1.write_to_doc(DocumentType::ClusterConfig, "cluster_name", b"demo-cluster").await?;
    transport1.write_to_doc(DocumentType::ClusterConfig, "cluster_version", b"1.0.0").await?;
    transport1.write_to_doc(DocumentType::ClusterConfig, "created_at", 
        chrono::Utc::now().to_rfc3339().as_bytes()).await?;
    ...
    for node_id in [1, 2, 3] {
        let status_key = format!("node_{}_status", node_id);
        match transport1.read_from_doc(DocumentType::ClusterConfig, &status_key).await {
            Ok(data) => {
