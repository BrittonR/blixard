...
use blixard_core::{
    ...
    transport::{
        ...
        iroh_vm_service::{VmRequest, VmOperationRequest},
        iroh_protocol::{serialize_payload, deserialize_payload, RpcRequest, write_message, read_message},
    },
...
async fn run_client_tests(
    ...
    identity_registry.register_node(
        client_node_id,
...
async fn test_time_based_access(
    ...
    if hour >= 22 || hour <= 6 {
        println!("✅ Current time ({} UTC) is in maintenance window - deletion allowed", hour);
    } else {
        println!("❌ Current time ({} UTC) is outside maintenance window - deletion blocked", hour);
    }
}
