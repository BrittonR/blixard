...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let receiver_task = tokio::spawn(async move {
        ...
        transport2_clone.accept_connections(move |doc_type, data| {
            println!("[Transport 2] Received data for {:?}: {} bytes", doc_type, data.len());
            if let Ok(msg) = std::str::from_utf8(&data) {
                println!("[Transport 2] Message content: {}", msg);
            }
            let _ = tx.try_send((doc_type, data));
        }).await.expect("Failed to accept connections");
    });
    ...
    transport1.write_to_doc(DocumentType::VmImages, "test-key", b"test-value").await?;
    let value = transport1.read_from_doc(DocumentType::VmImages, "test-key").await?;
    ...
    let health_task = tokio::spawn(async move {
        transport2_health.accept_health_check_connections().await
    });
