...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info,iroh=debug")),
        )
        .init();
    ...
    transport1
        .write_to_doc(DocumentType::ClusterConfig, "cluster_name", b"production")
        .await?;
    transport1
        .write_to_doc(DocumentType::ClusterConfig, "version", b"1.0.0")
        .await?;
    transport1
        .write_to_doc(DocumentType::ClusterConfig, "leader", b"node-1")
        .await?;
    ...
    match transport2
        .read_from_doc(DocumentType::ClusterConfig, "cluster_name")
        .await
    {
        Ok(data) => {
            info!(
                "Node 2: Read cluster_name = {}",
                String::from_utf8_lossy(&data)
            );
        }
        Err(e) => {
            warn!(
                "Node 2: Failed to read cluster_name (may need more sync time): {}",
                e
            );
        }
    }
    ...
    // Add some VM images
    transport2
        .write_to_doc(DocumentType::VmImages, "ubuntu-22.04", b"sha256:abc123...")
        .await?;
    transport2
        .write_to_doc(DocumentType::VmImages, "debian-12", b"sha256:def456...")
        .await?;
    transport2
        .write_to_doc(DocumentType::VmImages, "alpine-3.18", b"sha256:ghi789...")
        .await?;
    ...
    transport1
        .join_doc_from_ticket(&vm_ticket, DocumentType::VmImages)
        .await?;
    ...
    transport1
        .write_to_doc(DocumentType::ClusterConfig, "leader", b"node-2")
        .await?;
    transport1
        .write_to_doc(
            DocumentType::ClusterConfig,
            "last_election",
            b"2024-01-25T10:30:00Z",
        )
        .await?;
    ...
    match transport2
        .read_from_doc(DocumentType::ClusterConfig, "leader")
        .await
    {
        Ok(data) => {
    ...
    transport1
        .create_or_join_doc(DocumentType::Metrics, true)
        .await?;
    let metrics_ticket = transport1.get_doc_ticket(DocumentType::Metrics).await?;
    transport2
        .join_doc_from_ticket(&metrics_ticket, DocumentType::Metrics)
        .await?;

    // Node 1 writes its metrics
    transport1
        .write_to_doc(DocumentType::Metrics, "node1_cpu_usage", b"45.2")
        .await?;
    transport1
        .write_to_doc(DocumentType::Metrics, "node1_memory_usage", b"2048")
        .await?;

    // Node 2 writes its metrics
    transport2
        .write_to_doc(DocumentType::Metrics, "node2_cpu_usage", b"67.8")
        .await?;
    transport2
        .write_to_doc(DocumentType::Metrics, "node2_memory_usage", b"3072")
        .await?;
    ...
    match transport1
        .send_to_peer(&addr2, DocumentType::ClusterConfig, b"Hello from Node 1!")
        .await
    {
