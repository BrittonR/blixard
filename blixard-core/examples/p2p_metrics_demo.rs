...
async fn main() -> BlixardResult<()> {
    ...
    let p2p_manager1 = Arc::new(
        P2pManager::new(1, &node1_dir, P2pConfig::default()).await?
    );
    let image_store1 = NixImageStore::new(
        1,
        p2p_manager1,
        &node1_dir,
        None,
    ...
    let metadata = image_store1.import_microvm(
        "metrics-test-vm",
        &test_file,
        None,
    ).await?;
    ...
    let (_path, stats) = image_store1.download_image(
        &metadata.id,
        Some(&node1_dir.join("downloads")),
    ).await?;
    ...
    let p2p_manager2 = Arc::new(
        P2pManager::new(2, &node2_dir, P2pConfig::default()).await?
    );
    let image_store2 = NixImageStore::new(
        2,
        p2p_manager2,
        &node2_dir,
        None,
    ...
    let metadata2 = image_store2.import_microvm(
        "similar-test-vm",
        &similar_file,
        None,
    ).await?;
