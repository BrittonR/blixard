...
async fn main() -> BlixardResult<()> {
    ...
    // Get node addresses
    let addr1 = NodeAddr::new(node1.node_id())
        .with_direct_addresses(node1.bound_sockets());
    let addr2 = NodeAddr::new(node2.node_id())
        .with_direct_addresses(node2.bound_sockets());
    let addr3 = NodeAddr::new(node3.node_id())
        .with_direct_addresses(node3.bound_sockets());
                        ...
                        if let Ok(conn) = connecting.await {
                            while let Ok((mut send, mut recv)) = conn.accept_bi().await {
                                ...
                                if let Ok((_header, payload)) = read_message(&mut recv).await {
                                    ...
                                    // Send acknowledgment
                                    let response = Bytes::from(format!("ACK from Node {}", node_id).into_bytes());
        ...
        Ok(conn) => {
            match conn.open_bi().await {
                Ok((mut send, mut recv)) => {
                    ...
                    match tokio::time::timeout(Duration::from_millis(100), read_message(&mut recv)).await {
