...
async fn test_connection_reuse() -> BlixardResult<()> {
    ...
    let start = Instant::now();
    let connection = endpoint1.connect(node_addr, b"test").await.map_err(|e| {
    ...
    if !stream_times.is_empty() {
        ...
        println!(
            "   Average stream creation: {:?} (much faster than new connections)",
            avg
        );
    }
...
async fn test_concurrent_connections() -> BlixardResult<()> {
    ...
    for i in 0..10 {
        ...
        let task = tokio::spawn(async move {
            ...
            let _conn = endpoint.connect(server_addr, b"test").await.map_err(|e| {
                blixard_core::error::BlixardError::Internal {
