...
async fn test_node_id_operations() -> BlixardResult<()> {
    ...
    for _ in 0..1000 {
        ...
        let start = Instant::now();
        let _parsed = id_string.parse::<iroh::NodeId>().map_err(|e| {
...
async fn test_local_endpoints() -> BlixardResult<()> {
    ...
    let creation_time = start.elapsed();
    println!(
        "   Created {} endpoints in {:?}",
        endpoints.len(),
        creation_time
    );
    println!(
        "   Average per endpoint: {:?}",
        creation_time / endpoints.len() as u32
    );
...
async fn test_message_codec() -> BlixardResult<()> {
    use blixard_core::transport::iroh_protocol::{generate_request_id, MessageHeader, MessageType};
    use std::mem::size_of;
