...
async fn main() -> BlixardResult<()> {
                            ...
                            tokio::spawn(async move {
                                while let Ok((mut send, mut recv)) = conn.accept_bi().await {
                                    if let Ok((_, payload)) = read_message(&mut recv).await {
                                        ...
                                        // Send response
                                        let response = Bytes::from(b"Hello from Node 1 (through NAT!)".to_vec());
                                        let _ = write_message(&mut send, MessageType::Response, generate_request_id(), &response).await;
                                    }
                                }
                            });
                        }
    ...
    println!("  Direct addresses: {:?}", node1_addr.direct_addresses().collect::<Vec<_>>());
            ...
            Ok(Ok(conn)) => {
                ...
                match conn.open_bi().await {
                    Ok((mut send, mut recv)) => {
                        // Send message
                        let msg = format!("Hello from Node 2 via NAT traversal (attempt {})", attempts);
    ...
    let node2_addr = NodeAddr::new(node2_id)
        .with_relay_url("https://relay.iroh.network/".parse().unwrap());
        ...
        while let Some(incoming) = ep2_clone.accept().await {
            if let Ok(connecting) = incoming.accept() {
                if let Ok(conn) = connecting.await {
                    println!("✓ Node 2: Connection accepted from {:?}", conn.remote_node_id());
                    
                    tokio::spawn(async move {
                        while let Ok((mut send, mut recv)) = conn.accept_bi().await {
                            if let Ok((_, payload)) = read_message(&mut recv).await {
                                println!("  Node 2 received: {}", String::from_utf8_lossy(&payload));
    ...
    match timeout(Duration::from_secs(10), endpoint1.connect(node2_addr, b"blixard/1")).await {
        Ok(Ok(conn)) => {
