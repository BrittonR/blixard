...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add roles
    cedar
        .add_entity("Role", "admin", HashMap::new(), vec![])
        .await?;
    cedar
        .add_entity("Role", "operator", HashMap::new(), vec![])
        .await?;
    cedar
        .add_entity("Role", "viewer", HashMap::new(), vec![])
        .await?;
    ...
    cedar
        .add_entity("Tenant", "acme", tenant_attrs, vec![])
        .await?;
    ...
    cedar
        .add_entity("Cluster", "prod", cluster_attrs, vec![])
        .await?;
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "\n1. Can Alice (admin) read the cluster? {}",
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "2. Can Alice (admin) manage the cluster? {}",
        if result { "✅ YES" } else { "❌ NO" }
    );
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "3. Can Bob (operator) manage the cluster? {}",
        if result { "✅ YES" } else { "❌ NO" }
    );
    ...
    cedar
        .add_entity("VM", "vm1", vm_attrs, vec!["Tenant::\"acme\"".to_string()])
        .await?;
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "4. Can Bob delete VM at 2 PM? {}",
        if result {
            "✅ YES"
        } else {
            "❌ NO (maintenance window only)"
        }
    );
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "5. Can Bob delete VM at 11 PM? {}",
        if result {
            "✅ YES (maintenance window)"
        } else {
            "❌ NO"
        }
    );
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "6. Can Bob create VM (45/50 VMs, 800/1000 CPU)? {}",
        if result { "✅ YES" } else { "❌ NO" }
    );
    ...
    cedar
        .add_entity("Tenant", "competitor", other_tenant_attrs, vec![])
        .await?;
    ...
    let result = cedar
        ...
        .await?;
    println!(
        "8. Can Bob read competitor's VM? {}",
        if result {
            "✅ YES"
        } else {
            "❌ NO (tenant isolation)"
