...
async fn main() -> BlixardResult<()> {
    ...
    let p2p_manager = Arc::new(
        P2pManager::new(1, data_dir, P2pConfig::default()).await?
    );
    ...
    if let Ok(entries) = glob::glob(&nix_hello.to_string_lossy()) {
        for entry in entries.filter_map(Result::ok).take(1) {
            ...
            let nix_metadata = image_store.import_microvm(
                "nix-hello-test",
                &entry,
                None,
            ).await?;
    ...
    info!("   To compute NAR hash: nix-store --dump /path | nix-hash --type sha256 --base32 --flat");
