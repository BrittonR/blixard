...
fn benchmark_serialization(messages: &[Message]) {
    ...
    println!("Average message size: {} bytes", total_bytes / messages.len());
    println!("Throughput: {:.2} MB/s", (total_bytes as f64 / 1024.0 / 1024.0) / elapsed.as_secs_f64());
}
...
fn show_message_sizes(messages: &[Message]) {
    ...
    for msg in messages {
        if let Ok(bytes) = serialize_message(msg) {
            sizes_by_type.entry(msg.msg_type()).or_default().push(bytes.len());
        }
    }
