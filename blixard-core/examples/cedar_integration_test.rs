...
async fn main() -> BlixardResult<()> {
    ...
    let result = security_manager
        .check_permission_cedar("admin", "readCluster", "Cluster::\"prod\"", context.clone())
        .await?;

    println!(
        "1. Admin reading cluster: {}",
        if result { "✅ ALLOWED" } else { "❌ DENIED" }
    );

    // Test 2: Operator creating VM
    let result = security_manager
        .check_permission_cedar("operator", "createVM", "Node::\"node-1\"", context.clone())
        .await?;

    println!(
        "2. Operator creating VM: {}",
        if result { "✅ ALLOWED" } else { "❌ DENIED" }
    );

    // Test 3: Viewer trying to delete VM
    let result = security_manager
        .check_permission_cedar("viewer", "deleteVM", "VM::\"vm-123\"", context.clone())
        .await?;

    println!(
        "3. Viewer deleting VM: {}",
        if result { "✅ ALLOWED" } else { "❌ DENIED" }
    );
    ...
    let result = security_manager
        .check_permission_cedar("operator", "deleteVM", "VM::\"vm-123\"", context.clone())
        .await?;

    println!(
        "4. Operator deleting VM at 2 PM: {}",
        if result { "✅ ALLOWED" } else { "❌ DENIED" }
    );
    ...
    let result = security_manager
        .check_permission_cedar("operator", "deleteVM", "VM::\"vm-123\"", context)
        .await?;

    println!(
        "5. Operator deleting VM at 11 PM: {}",
        if result { "✅ ALLOWED" } else { "❌ DENIED" }
    );
